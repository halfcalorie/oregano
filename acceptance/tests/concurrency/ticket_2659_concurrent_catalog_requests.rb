...
with_oregano_running_on(master, master_opts, testdir) do
  ...
  agents.each do |agent|
    cert_path    = on(agent, oregano('config', 'print', 'hostcert')).stdout.chomp
    key_path     = on(agent, oregano('config', 'print', 'hostprivkey')).stdout.chomp
    cacert_path  = on(agent, oregano('config', 'print', 'localcacert')).stdout.chomp
    agent_cert   = on(agent, oregano('config', 'print', 'certname')).stdout.chomp
    ...
    create_remote_file(agent, test_script, <<-EOF)
      ...
      for (( i=0; i<$loops; i++ )); do
        (
          ...
          sleep $sleep_for
          url='https://#{master}:8140/oregano/v3/catalog/#{agent_cert}?environment=production'
          echo "Curling: $url"
