...
def setting_on(host, section, name)
  on(host, oregano("config", "print", name, "--section", section)).stdout.chomp
end
...
with_oregano_running_on(master, master_opts) do
  step "Ensure that an unauthenticated client cannot access the environments list" do
    on master, "curl --tlsv1 -ksv https://#{master}:#{master_port(master)}/oregano/v3/environments", :acceptable_exit_codes => [0,7] do
      assert_match(/< HTTP\/1\.\d 403/, stderr)
    end
  end

  step "Ensure that an authenticated client can retrieve the list of environments" do
    curl_master_from(master, '/oregano/v3/environments') do
