...
function validate_this() {
  notice('hello, oregano')
}
validate_this()
        SITE
        on(agent, oregano("parser validate --environment #{tmp_environment}"), :pty => true) # default manifest
      end
...
notice 42 =~ MyInteger
      ...
      on(agent, oregano("parser validate #{tmp_manifest}"))
    end
    ...
    step 'manifest with bad syntax' do
      ...
      on(agent, oregano("parser validate #{tmp_manifest}"), :accept_all_exit_codes => true) do |result|
