test_name "oregano module install (with unsatisfied constraints)"
...
on master, oregano("module install #{module_author}-#{module_name}"), :acceptable_exit_codes => [1] do
  ...
  assert_match(/Use `oregano module install --ignore-dependencies/, stderr,
...
on master, oregano("module install #{module_author}-#{module_name} --ignore-dependencies") do
...
on master, oregano("module install #{module_author}-stdlub --version 1.x"), :acceptable_exit_codes => [1] do
  assert_match(/No version.* can satisfy all dependencies/, stderr,
...
on master, oregano("module install #{module_author}-stdlub"), :acceptable_exit_codes => [1] do
  assert_match(/No version.* can satisfy all dependencies/, stderr,
        "Unsatisfiable dependency was not displayed")
end
assert_module_not_installed_on_disk(master, 'stdlub')
...
on master, oregano("module install #{module_author}-stdlub --force") do
