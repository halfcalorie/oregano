test_name "oregano module uninstall (with active dependency)"
...
on master, oregano('module uninstall jimmy-crakorn'), :acceptable_exit_codes => [1] do
  pattern = Regexp.new([
    ...
    %Q{    Use `oregano module uninstall --force` to uninstall this module anyway.*},
  ].join("\n"), Regexp::MULTILINE)
...
on master, oregano('module uninstall jimmy-crakorn --version 0.x'), :acceptable_exit_codes => [1] do
  pattern = Regexp.new([
...
on master, oregano('module uninstall jimmy-crakorn --force') do
