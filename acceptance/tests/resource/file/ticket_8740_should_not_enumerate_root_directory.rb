...
agents.each do |agent|
  ...
  on(agent, oregano_resource('file'), :acceptable_exit_codes => [1]) do
    assert_match(%r{Listing all file instances is not supported.  Please specify a file or directory, e.g. oregano resource file /etc}, stderr)
  end

  ["/", "/etc"].each do |file|
    step "query '#{file}' directory, which should return single entry"
    on(agent, oregano_resource('file', file)) do
      files = stdout.scan(/^file \{ '([^']+)'/).flatten

      assert_equal(1, files.size, "oregano returned multiple files: #{files.join(', ')}")
      assert_match(file, files[0], "oregano did not return file")
    end
  end

  step "query file that does not exist, which should report the file is absent"
  on(agent, oregano_resource('file', '/this/does/notexist')) do
