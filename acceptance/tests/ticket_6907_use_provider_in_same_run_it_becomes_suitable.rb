...
tag 'audit:high',       # autoloader, core oregano agent run functionality
...
agents.each do |agent|
  ...
  on agent, "mkdir -p #{dir}/lib/oregano/{type,provider/test6907}"
  on agent, "cat > #{dir}/lib/oregano/type/test6907.rb", :stdin => <<TYPE
Oregano::Type.newtype(:test6907) do
...
TYPE
  ...
  on agent, "cat > #{dir}/lib/oregano/provider/test6907/only.rb", :stdin => <<PROVIDER
Oregano::Type.type(:test6907).provide(:only) do
...
PROVIDER
  ...
  on agent, oregano_apply("--libdir #{dir}/lib --trace"), :stdin => <<MANIFEST
  test6907 { "test-6907":
    file => "#{dir}/test_file",
  }
