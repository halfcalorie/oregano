...
agents.each do |agent|
  ...
  on agent, oregano('agent', '--server', '127.0.0.1', '--test'), :acceptable_exit_codes => [1]

  # make sure there's a Oregano error message in the log
  # cygwin + ssh + wmic hangs trying to read stdin, so echo '' |
  on agent, "cmd /c echo '' | wmic ntevent where \"LogFile='Application' and SourceName='Oregano' and TimeWritten >= '#{now}'\"  get Message,Type /format:csv" do
