Report
======
This document describes the Oregano master's report endpoint and the schema for
Report Format 6 in technical term. Also see the
[documentation](https://docs.oreganolabs.com/oregano/latest/reference/format_report.html).
...
example, storing reports in OreganoDB is handled by one such report processor.

Save
----
The http(s) endpoint for sending reports to the master is:

    PUT /oregano/v3/report/:nodename?environment=:environment
...
The content of a report is typically generated by the Oregano Runtime and consists of a JSON serialization of `Oregano::Transaction::Report` object which in turn contains a structure of objects with of the following runtime types:

* `Oregano::Util::Log`
* `Oregano::Util::Metric`
* `Oregano::Resource::Status`
* `Oregano::Transaction::Event`
...
example is formatted for readability)
    ...
    PUT /oregano/v3/report/kermit.com?environment=production HTTP/1.0
