...
describe "autosigning" do
  ...
  let(:oregano_dir) { tmpdir("ca_autosigning") }
  ...
  let(:host) { Oregano::SSL::Host.new }

  before do
    Oregano.settings[:confdir] = oregano_dir
    Oregano.settings[:vardir] = oregano_dir
  ...
  def write_csr_attributes(yaml)
    File.open(Oregano.settings[:csr_attributes], 'w') do |file|
  ...
  context "when the csr_attributes file is valid, but empty" do
    it "generates a CSR when the file is empty" do
      Oregano::FileSystem.touch(Oregano.settings[:csr_attributes])
  ...
  context "when the csr_attributes file doesn't contain a YAML encoded hash" do
    it "raises when the file contains a string" do
      ...
      expect {
        host.generate_certificate_request
      }.to raise_error(Oregano::Error, /invalid CSR attributes, expected instance of Hash, received instance of String/)
    end
    ...
    it "raises when the file contains an empty array" do
      ...
      expect {
        host.generate_certificate_request
      }.to raise_error(Oregano::Error, /invalid CSR attributes, expected instance of Hash, received instance of Array/)
    end
  end
  ...
  context "with extension requests from csr_attributes file" do
    ...
    context "and subjectAltName" do
      it "raises an error if you include subjectAltName in csr_attributes" do
        ...
        write_csr_attributes(csr_attributes_content)
        expect { host.generate_certificate_request }.to raise_error(Oregano::Error, /subjectAltName.*conflicts with internally used extension request/)
      end

      it "properly merges subjectAltName when in settings" do
        Oregano.settings[:dns_alt_names] = 'althostname.nowhere'
        write_csr_attributes(csr_attributes_content)
        host.generate_certificate_request
        csr = Oregano::SSL::CertificateRequest.indirection.find(host.name)
    ...
    context "without subjectAltName" do
      ...
      it "does not copy custom attributes to certificate" do
        ...
        cert.custom_extensions.each do |ext|
          expect(Oregano::SSL::Oids.subtree_of?('1.3.6.1.4.1.34380.2', ext['oid'])).to be_falsey
