...
describe Oregano::Transaction::Report do
  describe "when using the indirector" do
    ...
    it "should be able to delegate to the :processor terminus" do
      Oregano::Transaction::Report.indirection.stubs(:terminus_class).returns :processor

      terminus = Oregano::Transaction::Report.indirection.terminus(:processor)
  ...
  describe "when dumping to YAML" do
    it "should not contain TagSet objects" do
      resource = Oregano::Resource.new(:notify, "Hello")
      ral_resource = resource.to_ral
      status = Oregano::Resource::Status.new(ral_resource)

      log = Oregano::Util::Log.new(:level => :info, :message => "foo")

      report = Oregano::Transaction::Report.new
      report.add_resource_status(status)
      report << log

      expect(YAML.dump(report)).to_not match('Oregano::Util::TagSet')
    end
  end
  ...
  describe "inference checking" do
    ...
    def run_catalogs(resources1, resources2, noop1 = false, noop2 = false, &block)
      last_run_report = nil
      Oregano::Transaction::Report.indirection.expects(:save).twice.with do |report, x|
    ...
    def new_blank_catalog
      Oregano::Resource::Catalog.new("testing", Oregano.lookup(:environments).get(Oregano[:environment]))
    end
    ...
    describe "for agent runs that contain" do
      it "notifies with catalog change" do
        report = run_catalogs(Oregano::Type.type(:notify).new(:title => "testing",
                                                             :message => "foo"),
                              Oregano::Type.type(:notify).new(:title => "testing",
                                                             :message => "foobar"))
      ...
      it "new file resource" do
        ...
        report = run_catalogs([],
                              Oregano::Type.type(:file).new(:title => file,
                                                           :content => "mystuff"))
      ...
      it "removal of a file resource" do
        file = tmpfile("test_file")
        report = run_catalogs(Oregano::Type.type(:file).new(:title => file,
                                                           :content => "mystuff"),
                              [])
      ...
      it "file with a title change" do
        ...
        report = run_catalogs(Oregano::Type.type(:file).new(:title => file1,
                                                           :content => "mystuff"),
                              Oregano::Type.type(:file).new(:title => file2,
                                                           :content => "mystuff"))
      ...
      it "link with ensure property change present => absent", :unless => Oregano.features.microsoft_windows? do
        file = tmpfile("test_file")
      ...
      it "exec with idempotence issue", :unless => Oregano.features.microsoft_windows? do
        report = run_catalogs(Oregano::Type.type(:exec).new(:title => "exec1",
                                                           :command => "/bin/echo foo"),
                              Oregano::Type.type(:exec).new(:title => "exec1",
                                                           :command => "/bin/echo foo"))
      ...
      it "exec with no idempotence issue", :unless => Oregano.features.microsoft_windows? do
        report = run_catalogs(Oregano::Type.type(:exec).new(:title => "exec1",
                                                           :command => "echo foo",
                                                           :path => "/bin",
                                                           :unless => "ls"),
      ...
      it "file skipped with file change between runs will not show corrective_change" do
        ...
        resources1 = [
          Oregano::Type.type(:file).new(:title => file,
                                       :content => "a",
                                       :notify => "Notify['foo']"),
          Oregano::Type.type(:notify).new(:title => "foo")
        ]
        resources2 = [
          Oregano::Type.type(:file).new(:title => file,
                                       :content => "a",
