...
describe Oregano::Type.type(:package), "when choosing a default package provider" do
  before do
  ...
  def provider_name(os)
    case os
    when 'Solaris'
      if Oregano::Util::Package.versioncmp(Facter.value(:kernelrelease), '5.11') >= 0
        :pkg
      else
        :sun
      end
    ...
    when 'Fedora'
      if Oregano::Util::Package.versioncmp(Facter.value(:operatingsystemmajrelease), '22') >= 0
        :dnf
      else
        :yum
      end
    when 'Suse'
      if Oregano::Util::Package.versioncmp(Facter.value(:operatingsystemmajrelease), '10') >= 0
        :zypper
      else
        :rug
      end
  ...
  it "should have a default provider" do
    expect(Oregano::Type.type(:package).defaultprovider).not_to be_nil
  end
  ...
  it "should choose the correct provider each platform" do
    ...
    expect(Oregano::Type.type(:package).defaultprovider.name).to eq(default_provider)
  end
end

describe Oregano::Type.type(:package), "when packages with the same name are sourced" do
  before :each do
    ...
    @package = Oregano::Type.type(:package).new(:name => "yay", :ensure => :present)
  ...
  describe "with same title" do
    before {
      @alt_package = Oregano::Type.type(:package).new(:name => "yay", :ensure => :present)
    }
    it "should give an error" do
      expect {
        @catalog.add_resource(@alt_package)
      }.to raise_error Oregano::Resource::Catalog::DuplicateResourceError, 'Duplicate declaration: Package[yay] is already declared; cannot redeclare'
    end
  end

  describe "with different title" do
    before :each do
      @alt_package = Oregano::Type.type(:package).new(:name => "yay", :title => "gem-yay", :ensure => :present)
    end

    it "should give an error" do
      provider_name = Oregano::Type.type(:package).defaultprovider.name
      expect {
        @catalog.add_resource(@alt_package)
  ...
  describe "from multiple providers" do
    ...
    before :each do
      ...
      @alt_package = Oregano::Type.type(:package).new(:name => "yay", :title => "gem-yay", :provider => @alt_provider, :ensure => :present)
      @catalog.add_resource(@alt_package)
    end
...
describe Oregano::Type.type(:package), 'logging package state transitions' do

  let(:catalog) { Oregano::Resource::Catalog.new }
  let(:provider) { stub('provider', :class => Oregano::Type.type(:package).defaultprovider, :clear => nil, :validate_source => nil) }
