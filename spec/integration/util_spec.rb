...
describe Oregano::Util do
  ...
  describe "#execute" do
    it "should properly allow stdout and stderr to share a file" do
      ...
      expect(Oregano::Util::Execution.execute(command, :combine => true).split).to match_array([*'1'..'10'])
    end

    it "should return output and set $CHILD_STATUS" do
      command = "ruby -e 'puts \"foo\"; exit 42'"

      output = Oregano::Util::Execution.execute(command, {:failonfail => false})
    ...
    it "should raise an error if non-zero exit status is returned" do
      ...
      expect { Oregano::Util::Execution.execute(command) }.to raise_error(Oregano::ExecutionFailure, /Execution of '#{command}' returned 43: /)
      expect($CHILD_STATUS.exitstatus).to eq(43)
    end
  end

  describe "#replace_file on Windows", :if => Oregano.features.microsoft_windows? do
    it "replace_file should preserve original ACEs from existing replaced file on Windows" do
      ...
      dacl.allow(admins, Oregano::Util::Windows::File::FILE_ALL_ACCESS)
      protect = true
      expected_sd = Oregano::Util::Windows::SecurityDescriptor.new(admins, admins, dacl, protect)
      Oregano::Util::Windows::Security.set_security_descriptor(file, expected_sd)

      ignored_mode = 0644
      Oregano::Util.replace_file(file, ignored_mode) do |temp_file|
        ignored_sd = Oregano::Util::Windows::Security.get_security_descriptor(temp_file.path)
        users = 'S-1-5-11'
        ignored_sd.dacl.allow(users, Oregano::Util::Windows::File::FILE_GENERIC_READ)
        Oregano::Util::Windows::Security.set_security_descriptor(temp_file.path, ignored_sd)
      end

      replaced_sd = Oregano::Util::Windows::Security.get_security_descriptor(file)
    ...
    it "replace_file should use reasonable default ACEs on a new file on Windows" do
      ...
      protected_sd = Oregano::Util::Windows::Security.get_security_descriptor(dir)
      protected_sd.protect = true
      Oregano::Util::Windows::Security.set_security_descriptor(dir, protected_sd)

      sibling_path = File.join(dir, 'sibling_file')
      FileUtils.touch(sibling_path)

      expected_sd = Oregano::Util::Windows::Security.get_security_descriptor(sibling_path)

      new_file_path = File.join(dir, 'new_file')

      ignored_mode = nil
      Oregano::Util.replace_file(new_file_path, ignored_mode) { |tmp_file| }

      new_sd = Oregano::Util::Windows::Security.get_security_descriptor(new_file_path)
    ...
    it "replace_file should work with filenames that include - and . (PUP-1389)" do
      ...
      Oregano::Util.replace_file(destination_file, nil) do |temp_file|
          temp_file.open
  ...
  describe "#which on Windows", :if => Oregano.features.microsoft_windows? do
    ...
    it "should be able to use UTF8 characters in the path" do
      ...
      Oregano::Util.withenv( { "PATH" => path } , :windows) do
        expect(Oregano::Util.which(filename)).to eq(filepath)
      end
    end
  end
