...
describe Oregano::Application::Cert => true do
  ...
  describe "when running" do
    before :each do
      ...
      Oregano::SSL::CertificateAuthority::Interface.stubs(:new).returns(@iface)
    end
    ...
    it "should delegate with :all if option --all was given" do
      ...
      Oregano::SSL::CertificateAuthority::Interface.expects(:new).returns(@iface).with { |cert_mode,to| to[:to] == :all }
    ...
    it "should delegate to ca.apply with the hosts given on command line" do
      ...
      Oregano::SSL::CertificateAuthority::Interface.expects(:new).returns(@iface).with { |cert_mode,to| to[:to] == ["host"]}
    ...
    it "should send the currently set digest" do
      ...
      Oregano::SSL::CertificateAuthority::Interface.expects(:new).returns(@iface).with { |cert_mode,to| to[:digest] == :digest}
    ...
    it "should revoke cert if cert_mode is clean" do
      ...
      Oregano::SSL::CertificateAuthority::Interface.expects(:new).returns(@iface).with { |cert_mode,to| cert_mode == :revoke }
      Oregano::SSL::CertificateAuthority::Interface.expects(:new).returns(@iface).with { |cert_mode,to| cert_mode == :destroy }
