...
require 'oregano/configurer/fact_handler'
require 'matchers/json'

class FactHandlerTester
  include Oregano::Configurer::FactHandler
...
describe Oregano::Configurer::FactHandler do
  ...
  describe "when finding facts" do
    it "should use the node name value to retrieve the facts" do
      foo_facts = Oregano::Node::Facts.new('foo')
      bar_facts = Oregano::Node::Facts.new('bar')
      Oregano::Node::Facts.indirection.save(foo_facts)
      Oregano::Node::Facts.indirection.save(bar_facts)
      Oregano[:certname] = 'foo'
      Oregano[:node_name_value] = 'bar'
    ...
    it "should fail if finding facts fails" do
      Oregano::Node::Facts.indirection.expects(:find).raises RuntimeError

      expect { facthandler.find_facts }.to raise_error(Oregano::Error, /Could not retrieve local facts/)
    end

    it "should only load fact plugins once" do
      Oregano::Node::Facts.indirection.expects(:find).once
      facthandler.find_facts
    end
  end
  ...
  context "when serializing" do
    ...
    context "as pson" do
      ...
      it "should serialize and CGI escape the fact values for uploading" do
        facts = Oregano::Node::Facts.new(Oregano[:node_name_value], 'my_name_fact' => 'other_node_name')
        Oregano::Node::Facts.indirection.save(facts)
        text = Oregano::Util.uri_query_encode(facthandler.find_facts.render(:pson))
      ...
      facts_with_special_characters.each do |test_fact|
        it "should properly accept the fact #{test_fact[:hash]}" do
          facts = Oregano::Node::Facts.new(Oregano[:node_name_value], test_fact[:hash])
          Oregano::Node::Facts.indirection.save(facts)
          text = Oregano::Util.uri_query_encode(facthandler.find_facts.render(:pson))
    ...
    context "as json" do
      it "should serialize and CGI escape the fact values for uploading" do
        facts = Oregano::Node::Facts.new(Oregano[:node_name_value], 'my_name_fact' => 'other_node_name')
        Oregano::Node::Facts.indirection.save(facts)
        text = Oregano::Util.uri_query_encode(facthandler.find_facts.render(:json))
