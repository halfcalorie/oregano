...
require 'oregano/confine/variable'

describe Oregano::Confine::Variable do
  it "should be named :variable" do
    expect(Oregano::Confine::Variable.name).to eq(:variable)
  end

  it "should require a value" do
    expect { Oregano::Confine::Variable.new }.to raise_error(ArgumentError)
  end

  it "should always convert values to an array" do
    expect(Oregano::Confine::Variable.new("/some/file").values).to be_instance_of(Array)
  end

  it "should have an accessor for its name" do
    expect(Oregano::Confine::Variable.new(:bar)).to respond_to(:name)
  end

  describe "when testing values" do
    before do
      @confine = Oregano::Confine::Variable.new("foo")
      @confine.name = :myvar
    end

    it "should use settings if the variable name is a valid setting" do
      Oregano.settings.expects(:valid?).with(:myvar).returns true
      Oregano.settings.expects(:value).with(:myvar).returns "foo"
      @confine.valid?
    end

    it "should use Facter if the variable name is not a valid setting" do
      Oregano.settings.expects(:valid?).with(:myvar).returns false
    ...
    it "should produce a message that the fact value is not correct" do
      @confine = Oregano::Confine::Variable.new(%w{bar bee})
      @confine.name = "eh"
  ...
  describe "when summarizing multiple instances" do
    it "should return a hash of failing variables and their values" do
      c1 = Oregano::Confine::Variable.new("one")
      c1.name = "uno"
      c1.expects(:valid?).returns false
      c2 = Oregano::Confine::Variable.new("two")
      c2.name = "dos"
      c2.expects(:valid?).returns true
      c3 = Oregano::Confine::Variable.new("three")
      c3.name = "tres"
      c3.expects(:valid?).returns false

      expect(Oregano::Confine::Variable.summarize([c1, c2, c3])).to eq({"uno" => %w{one}, "tres" => %w{three}})
    end
    ...
    it "should combine the values of multiple confines with the same fact" do
      ...
      expect(Oregano::Confine::Variable.summarize([c1, c2])).to eq({"uno" => %w{one two}})
    end
  end
end
