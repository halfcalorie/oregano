...
describe Oregano::Face[:generate, :current] do
  ...
  # * Produces all types found on the module path (that are not in oregano core)
    ...
    context "in an environment with two modules containing resource types" do
      ...
      around(:each) do |example|
        ...
        Oregano.override(:environments => loader) do
          Oregano.override(:current_environment => loader.get('testing_generate')) do
            example.run
          end
        end
      end
      ...
      it 'sets pcore as the default format' do
        Oregano::Generate::Type.expects(:find_inputs).with(:pcore).returns([])
        genface.types()
      end
      ...
      it 'finds all files to generate types for' do
        ...
        input1 = Oregano::Generate::Type::Input.new(m1, File.join(m1, 'lib', 'oregano', 'type', 'test1.rb'), :pcore)
        input2 = Oregano::Generate::Type::Input.new(m1, File.join(m2, 'lib', 'oregano', 'type', 'test2.rb'), :pcore)
        Oregano::Generate::Type::Input.expects(:new).with(m1, File.join(m1, 'lib', 'oregano', 'type', 'test1.rb'), :pcore).returns(input1)
        Oregano::Generate::Type::Input.expects(:new).with(m2, File.join(m2, 'lib', 'oregano', 'type', 'test2.rb'), :pcore).returns(input2)
        genface.types
      end

      it 'creates output directory <env>/.resource_types/ if it does not exist' do
        expect(Oregano::FileSystem.exist?(outputdir)).to be(false)
        genface.types
        expect(Oregano::FileSystem.dir_exist?(outputdir)).to be(true)
      end

      it 'creates output with matching names for each input' do
        expect(Oregano::FileSystem.exist?(outputdir)).to be(false)
        genface.types
        children = Oregano::FileSystem.children(outputdir).map {|p| Oregano::FileSystem.basename_string(p) }
        expect(children.sort).to eql(['test1.pp', 'test2.pp'])
      end

      it 'tolerates that <env>/.resource_types/ directory exists' do
        Oregano::FileSystem.mkpath(outputdir)
        expect(Oregano::FileSystem.exist?(outputdir)).to be(true)
        genface.types
        expect(Oregano::FileSystem.dir_exist?(outputdir)).to be(true)
      end

      it 'errors if <env>/.resource_types exists and is not a directory' do
        expect(Oregano::FileSystem.exist?(outputdir)).to be(false) # assert it is not already there
        Oregano::FileSystem.touch(outputdir)
        expect(Oregano::FileSystem.exist?(outputdir)).to be(true)
        expect(Oregano::FileSystem.directory?(outputdir)).to be(false)
        expect {
          genface.types
      ...
      it 'does not overwrite if files exists and are up to date' do
        ...
        genface.types
        stats_before = [Oregano::FileSystem.stat(File.join(outputdir, 'test1.pp')), Oregano::FileSystem.stat(File.join(outputdir, 'test2.pp'))]
        # generate again
        genface.types
        stats_after = [Oregano::FileSystem.stat(File.join(outputdir, 'test1.pp')), Oregano::FileSystem.stat(File.join(outputdir, 'test2.pp'))]
      ...
      it 'overwrites if files exists that are not up to date while keeping up to date files' do
        ...
        sleep(1)
        Oregano::FileSystem.touch(File.join(m1, 'lib', 'oregano', 'type', 'test1.rb'))
        # generate again
        genface.types
      ...
      it 'removes previously generated files from output when there is no input for it' do
        # create them (first run)
        genface.types
        stat_before = Oregano::FileSystem.stat(File.join(outputdir, 'test2.pp'))
        # remove input
        Oregano::FileSystem.unlink(File.join(m1, 'lib', 'oregano', 'type', 'test1.rb'))
        # generate again
        genface.types
        # assert that test1 was deleted but not test2 (same time)
        expect(Oregano::FileSystem.exist?(File.join(outputdir, 'test1.pp'))).to be(false)
        stats_after = Oregano::FileSystem.stat(File.join(outputdir, 'test2.pp'))
