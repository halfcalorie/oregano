...
describe Oregano::FileServing::Fileset do
  ...
  context "when initializing" do
    it "requires a path" do
      expect { Oregano::FileServing::Fileset.new }.to raise_error(ArgumentError)
    end

    it "fails if its path is not fully qualified" do
      expect { Oregano::FileServing::Fileset.new("some/file") }.to raise_error(ArgumentError, "Fileset paths must be fully qualified: some/file")
    end

    it "removes a trailing file path separator" do
      path_with_separator = "#{somefile}#{File::SEPARATOR}"
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      fileset = Oregano::FileServing::Fileset.new(path_with_separator)
      expect(fileset.path).to eq(somefile)
    end

    it "can be created from the root directory" do
      path = File.expand_path(File::SEPARATOR)
      Oregano::FileSystem.expects(:lstat).with(path).returns stub('stat')
      fileset = Oregano::FileServing::Fileset.new(path)
      expect(fileset.path).to eq(path)
    end

    it "fails if its path does not exist" do
      Oregano::FileSystem.expects(:lstat).with(somefile).raises(Errno::ENOENT)
      expect { Oregano::FileServing::Fileset.new(somefile) }.to raise_error(ArgumentError, "Fileset paths must exist")
    end

    it "accepts a 'recurse' option" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      set = Oregano::FileServing::Fileset.new(somefile, :recurse => true)
      expect(set.recurse).to be_truthy
    end

    it "accepts a 'recurselimit' option" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      set = Oregano::FileServing::Fileset.new(somefile, :recurselimit => 3)
      expect(set.recurselimit).to eq(3)
    end

    it "accepts an 'ignore' option" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      set = Oregano::FileServing::Fileset.new(somefile, :ignore => ".svn")
      expect(set.ignore).to eq([".svn"])
    end

    it "accepts a 'links' option" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      set = Oregano::FileServing::Fileset.new(somefile, :links => :manage)
      expect(set.links).to eq(:manage)
    end

    it "accepts a 'checksum_type' option" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      set = Oregano::FileServing::Fileset.new(somefile, :checksum_type => :test)
      expect(set.checksum_type).to eq(:test)
    end

    it "fails if 'links' is set to anything other than :manage or :follow" do
      expect { Oregano::FileServing::Fileset.new(somefile, :links => :whatever) }.to raise_error(ArgumentError, "Invalid :links value 'whatever'")
    end

    it "defaults to 'false' for recurse" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      expect(Oregano::FileServing::Fileset.new(somefile).recurse).to eq(false)
    end

    it "defaults to :infinite for recurselimit" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      expect(Oregano::FileServing::Fileset.new(somefile).recurselimit).to eq(:infinite)
    end

    it "defaults to an empty ignore list" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      expect(Oregano::FileServing::Fileset.new(somefile).ignore).to eq([])
    end

    it "defaults to :manage for links" do
      Oregano::FileSystem.expects(:lstat).with(somefile).returns stub('stat')
      expect(Oregano::FileServing::Fileset.new(somefile).links).to eq(:manage)
    end
    ...
    describe "using an indirector request" do
      ...
      [:recurse, :recurselimit, :ignore, :links].each do |option|
        it "passes the #{option} option on to the fileset if present" do
          request = Oregano::Indirector::Request.new(:file_serving, :find, "foo", nil, {option => values[option]})

          expect(Oregano::FileServing::Fileset.new(somefile, request).send(option)).to eq(values[option])
        end
      end

      it "converts the integer as a string to their integer counterpart when setting options" do
        request = Oregano::Indirector::Request.new(:file_serving, :find, "foo", nil,
                                                  {:recurselimit => "1234"})

        expect(Oregano::FileServing::Fileset.new(somefile, request).recurselimit).to eq(1234)
      end
      ...
      it "converts the string 'true' to the boolean true when setting options" do
        ...
        expect(Oregano::FileServing::Fileset.new(somefile, request).recurse).to eq(true)
      end
      ...
      it "converts the string 'false' to the boolean false when setting options" do
        ...
        expect(Oregano::FileServing::Fileset.new(somefile, request).recurse).to eq(false)
      end
    end
  end
  ...
  context "when recursing" do
    before do
      ...
      Oregano::FileSystem.stubs(:lstat).with(@path).returns stub('stat', :directory? => true)
    ...
    def mock_dir_structure(path, stat_method = :lstat)
      Oregano::FileSystem.stubs(stat_method).with(path).returns @dirstat
      ...
      top_names.each do |subdir|
        ...
        Oregano::FileSystem.stubs(stat_method).with(subpath).returns @dirstat
        ...
        sub_names.each do |file|
          ...
          Oregano::FileSystem.stubs(stat_method).with(subfile_path).returns(@filestat)
        end
      end
    end
    ...
    MockDirectory = Struct.new(:name, :entries) do
      def mock(base_path)
        ...
        Oregano::FileSystem.stubs(:lstat).with(path).returns MockStat.new(path, true)
    ...
    MockFile = Struct.new(:name) do
      def mock(base_path)
        extend Mocha::API
        path = File.join(base_path, name)
        Oregano::FileSystem.stubs(:lstat).with(path).returns MockStat.new(path, false)
      end
    end
    ...
    it "doesn't ignore pending directories when the last entry at the top level is a file" do
      ...
      fileset = Oregano::FileServing::Fileset.new(make_absolute('/your/path'))
    ...
    it "uses Oregano::FileSystem#stat if :links is set to :follow" do
    ...
    it "uses Oregano::FileSystem#lstat if :links is set to :manage" do
    ...
    it "works when paths have regexp significant characters" do
      ...
      Oregano::FileSystem.expects(:lstat).with(@path).returns stub(@path, :stat => stat, :lstat => stat)
      @fileset = Oregano::FileServing::Fileset.new(@path)
      mock_dir_structure(@path)
      @fileset.recurse = true
      expect(@fileset.files.sort).to eq(@files.sort)
  ...
  it "manages the links to missing files" do
    ...
    Oregano::FileSystem.expects(:stat).with(path).returns stat
    Oregano::FileSystem.expects(:lstat).with(path).returns stat

    link_path = File.join(path, "mylink")
    Oregano::FileSystem.expects(:stat).with(link_path).raises(Errno::ENOENT)
  ...
  context "when merging other filesets" do
    before do
      ...
      Oregano::FileSystem.stubs(:lstat).returns stub('stat', :directory? => false)

      @filesets = @paths.collect do |path|
        Oregano::FileSystem.stubs(:lstat).with(path).returns stub('stat', :directory? => true)
        Oregano::FileServing::Fileset.new(path, :recurse => true)
      end
    ...
    it "returns a hash of all files in each fileset with the value being the base path" do
      ...
      expect(Oregano::FileServing::Fileset.merge(*@filesets)).to eq({
    ...
    it "includes the base directory from the first fileset" do
      ...
      expect(Oregano::FileServing::Fileset.merge(*@filesets)["."]).to eq(make_absolute("/first/path"))
    end
    ...
    it "uses the base path of the first found file when relative file paths conflict" do
      ...
      expect(Oregano::FileServing::Fileset.merge(*@filesets)["one"]).to eq(make_absolute("/first/path"))
    end
  end
end
