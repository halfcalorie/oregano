...
describe Oregano::FileSystem::PathPattern do
  include OreganoSpec::Files
  InvalidPattern = Oregano::FileSystem::PathPattern::InvalidPattern

  describe 'relative' do
    it "can not be created with a traversal up the directory tree" do
      expect do
        Oregano::FileSystem::PathPattern.relative("my/../other")
    ...
    it "can be created with a '..' prefixing a filename" do
      expect(Oregano::FileSystem::PathPattern.relative("my/..other").to_s).to eq("my/..other")
    end
    ...
    it "can be created with a '..' embedded in a filename" do
      expect(Oregano::FileSystem::PathPattern.relative("my/ot..her").to_s).to eq("my/ot..her")
    end

    it "can not be created with a \\0 byte embedded" do
      expect do
        Oregano::FileSystem::PathPattern.relative("my/\0/other")
    ...
    it "can not be created with a windows drive" do
      expect do
        Oregano::FileSystem::PathPattern.relative("c:\\relative\\path")
    ...
    it "can not create an absolute relative path" do
      expect do
        Oregano::FileSystem::PathPattern.relative("/no/absolutes")
    ...
    it "can not create a relative path that is a windows path relative to the current drive" do
      expect do
        Oregano::FileSystem::PathPattern.relative("\\no\relatives")
    ...
    it "creates a relative PathPattern from a valid relative path" do
      expect(Oregano::FileSystem::PathPattern.relative("a/relative/path").to_s).to eq("a/relative/path")
    end

    it "is not absolute" do
      expect(Oregano::FileSystem::PathPattern.relative("a/relative/path")).to_not be_absolute
    end
  end
  ...
  describe 'absolute' do
    it "can not create a relative absolute path" do
      expect do
        Oregano::FileSystem::PathPattern.absolute("no/relatives")
    ...
    it "can not create an absolute path that is a windows path relative to the current drive" do
      expect do
        Oregano::FileSystem::PathPattern.absolute("\\no\\relatives")
      end.to raise_error(InvalidPattern, "A PathPattern cannot be a Windows current drive relative path.")
    end

    it "creates an absolute PathPattern from a valid absolute path" do
      expect(Oregano::FileSystem::PathPattern.absolute("/an/absolute/path").to_s).to eq("/an/absolute/path")
    end

    it "creates an absolute PathPattern from a valid Windows absolute path" do
      expect(Oregano::FileSystem::PathPattern.absolute("c:/absolute/windows/path").to_s).to eq("c:/absolute/windows/path")
    end

    it "can be created with a '..' embedded in a filename on windows", :if => Oregano.features.microsoft_windows? do
      expect(Oregano::FileSystem::PathPattern.absolute(%q{c:\..my\ot..her\one..}).to_s).to eq(%q{c:\..my\ot..her\one..})
    end

    it "is absolute" do
      expect(Oregano::FileSystem::PathPattern.absolute("c:/absolute/windows/path")).to be_absolute
    end
  end

  it "prefixes the relative path pattern with another path" do
    pattern = Oregano::FileSystem::PathPattern.relative("docs/*_thoughts.txt")
    prefix = Oregano::FileSystem::PathPattern.absolute("/prefix")
  ...
  it "refuses to prefix with a relative pattern" do
    pattern = Oregano::FileSystem::PathPattern.relative("docs/*_thoughts.txt")
    prefix = Oregano::FileSystem::PathPattern.relative("prefix")
  ...
  it "applies the pattern to the filesystem as a glob" do
    ...
    pattern = Oregano::FileSystem::PathPattern.relative("found_*").prefix_with(
      Oregano::FileSystem::PathPattern.absolute(dir))
