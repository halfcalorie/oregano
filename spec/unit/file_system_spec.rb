...
describe "Oregano::FileSystem" do
  ...
  def is_current_user_system?
    SYSTEM_SID_BYTES == Oregano::Util::Windows::ADSI::User.current_user_sid.sid_bytes
  end
  ...
  context "#open" do
    it "uses the same default mode as File.open, when specifying a nil mode (umask used on non-Windows)" do
      ...
      Oregano::FileSystem.open(file, nil, 'a') { |fh| fh.write('') }

      expected_perms = Oregano::Util::Platform.windows? ?
      ...
      expect([expected_perms].flatten).to include(Oregano::FileSystem.stat(file).mode.to_s(8))

      default_file = tmpfile('file_to_update2')
      expect(Oregano::FileSystem.exist?(default_file)).to be_falsey

      File.open(default_file, 'a') { |fh| fh.write('') }

      # which matches the behavior of File.open
      expect(Oregano::FileSystem.stat(file).mode).to eq(Oregano::FileSystem.stat(default_file).mode)
    end

    it "can accept an octal mode integer" do
      file = tmpfile('file_to_update')
      # NOTE: 777 here returns 755, but due to Ruby?
      Oregano::FileSystem.open(file, 0444, 'a') { |fh| fh.write('') }
      ...
      expected_ruby_mode = Oregano::Util::Platform.windows? ?
    ...
    it "cannot accept a mode string" do
      file = tmpfile('file_to_update')
      expect {
        Oregano::FileSystem.open(file, "444", 'a') { |fh| fh.write('') }
    ...
    it "opens, creates ands allows updating of a new file, using by default, the external system encoding" do
      begin
        ...
        Oregano::FileSystem.open(file, 0660, 'w') do |fh|
        ...
        written = Oregano::FileSystem.read(file)
  ...
  context "#exclusive_open" do
    it "opens ands allows updating of an existing file" do
      ...
      Oregano::FileSystem.exclusive_open(file, 0660, 'r+') do |fh|
        old = fh.read
        fh.truncate(0)
        fh.rewind
      ...
      expect(Oregano::FileSystem.read(file)).to eq("updated the contents")
    end
    ...
    it "opens, creates ands allows updating of a new file, using by default, the external system encoding" do
      begin
        ...
        # test writing a UTF-8 string when Default external encoding is something different
        Oregano::FileSystem.exclusive_open(file, 0660, 'w') do |fh|
          # note Ruby behavior which has no external_encoding, but implicitly uses Encoding.default_external
          expect(fh.external_encoding).to be_nil
          # write a UTF-8 string to this file
          fh.write(mixed_utf8)
    ...
    it "excludes other processes from updating at the same time", :unless => Oregano::Util::Platform.windows? do
      file = file_containing("file_to_update", "0")

      increment_counter_in_multiple_processes(file, 5, 'r+')

      expect(Oregano::FileSystem.read(file)).to eq("5")
    end

    it "excludes other processes from updating at the same time even when creating the file", :unless => Oregano::Util::Platform.windows? do
      file = tmpfile("file_to_update")
    ...
    it "times out if the lock cannot be acquired in a specified amount of time", :unless => Oregano::Util::Platform.windows? do
      file = tmpfile("file_to_update")

      child = spawn_process_that_locks(file)

      expect do
        Oregano::FileSystem.exclusive_open(file, 0666, 'a', 0.1) do |f|
        end
    ...
    def spawn_process_that_locks(file)
      ...
      child = Kernel.fork do
        read.close
        Oregano::FileSystem.exclusive_open(file, 0666, 'a') do |fh|
          write.write(true)
          write.close
          sleep 10
        end
    ...
    def increment_counter_in_multiple_processes(file, num_procs, options)
      ...
      num_procs.times do
        children << Kernel.fork do
          Oregano::FileSystem.exclusive_open(file, 0660, options) do |fh|
            fh.rewind
  ...
  context "read_preserve_line_endings" do
    it "should read a file with line feed" do
      with_file_content("file content \n") do |file|
        expect(Oregano::FileSystem.read_preserve_line_endings(file)).to eq("file content \n")
      end
    end
  ...
  context "read without an encoding specified" do
    it "returns strings as Encoding.default_external" do
      ...
      contents = Oregano::FileSystem.read(temp_file)
  ...
  context "read should allow an encoding to be specified" do
    ...
    it "and should read a UTF8 file properly" do
      ...
      contents = Oregano::FileSystem.read(temp_file, :encoding => 'utf-8')
  ...
  describe "symlink",
    :if => ! Oregano.features.manages_symlinks? &&
    Oregano.features.microsoft_windows? do
    ...
    it "should raise an error when trying to create a symlink" do
      expect { Oregano::FileSystem.symlink(file, 'foo') }.to raise_error(Oregano::Util::Windows::Error)
    end

    it "should return false when trying to check if a path is a symlink" do
      expect(Oregano::FileSystem.symlink?(file)).to be_falsey
    end

    it "should raise an error when trying to read a symlink" do
      expect { Oregano::FileSystem.readlink(file) }.to raise_error(Oregano::Util::Windows::Error)
    end

    it "should return a File::Stat instance when calling stat on an existing file" do
      expect(Oregano::FileSystem.stat(file)).to be_instance_of(File::Stat)
    end

    it "should raise Errno::ENOENT when calling stat on a missing file" do
      expect { Oregano::FileSystem.stat(missing_file) }.to raise_error(Errno::ENOENT)
    end

    it "should fall back to stat when trying to lstat a file" do
      Oregano::Util::Windows::File.expects(:stat).with(Oregano::FileSystem.assert_path(file))

      Oregano::FileSystem.lstat(file)
    end
  end

  describe "symlink", :if => Oregano.features.manages_symlinks? do
    ...
    it "should return true for exist? on a present file" do
      expect(Oregano::FileSystem.exist?(file)).to be_truthy
    end

    it "should return true for file? on a present file" do
      expect(Oregano::FileSystem.file?(file)).to be_truthy
    end

    it "should return false for exist? on a non-existent file" do
      expect(Oregano::FileSystem.exist?(missing_file)).to be_falsey
    end

    it "should return true for exist? on a present directory" do
      expect(Oregano::FileSystem.exist?(dir)).to be_truthy
    end

    it "should return false for exist? on a dangling symlink" do
      symlink = tmpfile("somefile_link")
      Oregano::FileSystem.symlink(missing_file, symlink)

      expect(Oregano::FileSystem.exist?(missing_file)).to be_falsey
      expect(Oregano::FileSystem.exist?(symlink)).to be_falsey
    end

    it "should return true for exist? on valid symlinks" do
      [file, dir].each do |target|
        symlink = tmpfile("#{Oregano::FileSystem.basename(target).to_s}_link")
        Oregano::FileSystem.symlink(target, symlink)

        expect(Oregano::FileSystem.exist?(target)).to be_truthy
        expect(Oregano::FileSystem.exist?(symlink)).to be_truthy
      end
    end
    ...
    it "should return false for exist? when resolving a cyclic symlink chain" do
      # point symlink -> file
      symlink = tmpfile("somefile_link")
      Oregano::FileSystem.symlink(file, symlink)

      # point symlink2 -> symlink
      symlink2 = tmpfile("somefile_link2")
      Oregano::FileSystem.symlink(symlink, symlink2)

      # point symlink3 -> symlink2
      symlink3 = tmpfile("somefile_link3")
      Oregano::FileSystem.symlink(symlink2, symlink3)

      # yank file, temporarily dangle
      ::File.delete(file)

      # and trash it so that we can recreate it OK on windows
      Oregano::FileSystem.unlink(symlink)

      # point symlink -> symlink3 to create a cycle
      Oregano::FileSystem.symlink(symlink3, symlink)

      expect(Oregano::FileSystem.exist?(symlink3)).to be_falsey
    end
    ...
    it "should return true for exist? when resolving a symlink chain pointing to a file" do
      ...
      expect(Oregano::FileSystem.exist?(symlink3)).to be_truthy
    end
    ...
    it "should not create a symlink when the :noop option is specified" do
      [file, dir].each do |target|
        symlink = tmpfile("#{Oregano::FileSystem.basename(target)}_link")
        Oregano::FileSystem.symlink(target, symlink, { :noop => true })
    ...
    it "should raise Errno::EEXIST if trying to create a file / directory symlink when the symlink path already exists as a file" do
      existing_file = tmpfile("#{Oregano::FileSystem.basename(file)}_link")
      FileUtils.touch(existing_file)

      [file, dir].each do |target|
        expect { Oregano::FileSystem.symlink(target, existing_file) }.to raise_error(Errno::EEXIST)

        expect(Oregano::FileSystem.exist?(existing_file)).to be_truthy
        expect(Oregano::FileSystem.symlink?(existing_file)).to be_falsey
      end
    end
    ...
    it "should silently fail if trying to create a file / directory symlink when the symlink path already exists as a directory" do
      existing_dir = tmpdir("#{Oregano::FileSystem.basename(file)}_dir")

      [file, dir].each do |target|
        expect(Oregano::FileSystem.symlink(target, existing_dir)).to eq(0)

        expect(Oregano::FileSystem.exist?(existing_dir)).to be_truthy
        expect(File.directory?(existing_dir)).to be_truthy
        expect(Oregano::FileSystem.symlink?(existing_dir)).to be_falsey
      end
    end

    it "should silently fail to modify an existing directory symlink to reference a new file or directory" do
      [file, dir].each do |target|
        existing_dir = tmpdir("#{Oregano::FileSystem.basename(target)}_dir")
        symlink = tmpfile("#{Oregano::FileSystem.basename(existing_dir)}_link")
        Oregano::FileSystem.symlink(existing_dir, symlink)

        expect(Oregano::FileSystem.readlink(symlink)).to eq(Oregano::FileSystem.path_string(existing_dir))

        # now try to point it at the new target, no error raised, but file system unchanged
        expect(Oregano::FileSystem.symlink(target, symlink)).to eq(0)
        expect(Oregano::FileSystem.readlink(symlink)).to eq(existing_dir.to_s)
      end
    end

    it "should raise Errno::EEXIST if trying to modify a file symlink to reference a new file or directory" do
      symlink = tmpfile("#{Oregano::FileSystem.basename(file)}_link")
      file_2 = tmpfile("#{Oregano::FileSystem.basename(file)}_2")
      FileUtils.touch(file_2)
      # symlink -> file_2
      Oregano::FileSystem.symlink(file_2, symlink)

      [file, dir].each do |target|
        expect { Oregano::FileSystem.symlink(target, symlink) }.to raise_error(Errno::EEXIST)
        expect(Oregano::FileSystem.readlink(symlink)).to eq(file_2.to_s)
      end
    end
    ...
    it "should delete the existing file when creating a file / directory symlink with :force when the symlink path exists as a file" do
      [file, dir].each do |target|
        existing_file = tmpfile("#{Oregano::FileSystem.basename(target)}_existing")
        FileUtils.touch(existing_file)
        expect(Oregano::FileSystem.symlink?(existing_file)).to be_falsey

        Oregano::FileSystem.symlink(target, existing_file, { :force => true })

        expect(Oregano::FileSystem.symlink?(existing_file)).to be_truthy
        expect(Oregano::FileSystem.readlink(existing_file)).to eq(target.to_s)
      end
    end
    ...
    it "should modify an existing file symlink when using :force to reference a new file or directory" do
      [file, dir].each do |target|
        existing_file = tmpfile("#{Oregano::FileSystem.basename(target)}_existing")
        FileUtils.touch(existing_file)
        existing_symlink = tmpfile("#{Oregano::FileSystem.basename(existing_file)}_link")
        Oregano::FileSystem.symlink(existing_file, existing_symlink)

        expect(Oregano::FileSystem.readlink(existing_symlink)).to eq(existing_file.to_s)

        Oregano::FileSystem.symlink(target, existing_symlink, { :force => true })

        expect(Oregano::FileSystem.readlink(existing_symlink)).to eq(target.to_s)
      end
    end
    ...
    it "should silently fail if trying to overwrite an existing directory with a new symlink when using :force to reference a file or directory" do
      [file, dir].each do |target|
        existing_dir = tmpdir("#{Oregano::FileSystem.basename(target)}_existing")

        expect(Oregano::FileSystem.symlink(target, existing_dir, { :force => true })).to eq(0)
    ...
    it "should silently fail if trying to modify an existing directory symlink when using :force to reference a new file or directory" do
      [file, dir].each do |target|
        existing_dir = tmpdir("#{Oregano::FileSystem.basename(target)}_existing")
        existing_symlink = tmpfile("#{Oregano::FileSystem.basename(existing_dir)}_link")
        Oregano::FileSystem.symlink(existing_dir, existing_symlink)

        expect(Oregano::FileSystem.readlink(existing_symlink)).to eq(existing_dir.to_s)

        expect(Oregano::FileSystem.symlink(target, existing_symlink, { :force => true })).to eq(0)

        expect(Oregano::FileSystem.readlink(existing_symlink)).to eq(existing_dir.to_s)
      end
    end

    it "should accept a string, Pathname or object with to_str (Oregano::Util::WatchedFile) for exist?" do
      [ tmpfile('bogus1'),
        Pathname.new(tmpfile('bogus2')),
        Oregano::Util::WatchedFile.new(tmpfile('bogus3'))
        ].each { |f| expect(Oregano::FileSystem.exist?(f)).to be_falsey  }
    end
    ...
    it "should return a File::Stat with ftype 'link' when calling lstat on a symlink pointing to existing file" do
      ...
      stat = Oregano::FileSystem.lstat(symlink)
    ...
    it "should return a File::Stat of ftype 'file' when calling stat on a symlink pointing to existing file" do
      ...
      stat = Oregano::FileSystem.stat(symlink)
      expect(stat).to be_instance_of(File::Stat)
    ...
    it "should return a File::Stat of ftype 'directory' when calling stat on a symlink pointing to existing directory" do
      symlink = tmpfile("somefile_link")
      Oregano::FileSystem.symlink(dir, symlink)
    ...
    it "should return a File::Stat of ftype 'file' when calling stat on a symlink pointing to another symlink" do
      ...
      expect(Oregano::FileSystem.stat(symlink2).ftype).to eq('file')
    end
    ...
    it "should raise Errno::ENOENT when calling stat on a dangling symlink" do
      ...
      expect { Oregano::FileSystem.stat(symlink) }.to raise_error(Errno::ENOENT)
    end
    ...
    it "should be able to readlink to resolve the physical path to a symlink" do
      ...
      expect(Oregano::FileSystem.exist?(file)).to be_truthy
      expect(Oregano::FileSystem.readlink(symlink)).to eq(file.to_s)
    end
    ...
    it "should not resolve entire symlink chain with readlink on a symlink'd symlink" do
      ...
      expect(Oregano::FileSystem.exist?(file)).to be_truthy
      expect(Oregano::FileSystem.readlink(symlink2)).to eq(symlink.to_s)
    end
    ...
    it "should be able to readlink to resolve the physical path to a dangling symlink" do
      ...
      expect(Oregano::FileSystem.exist?(missing_file)).to be_falsey
      expect(Oregano::FileSystem.readlink(symlink)).to eq(missing_file.to_s)
    end
    ...
    it "should be able to unlink a dangling symlink pointed at a file" do
      ...
      expect(Oregano::FileSystem).to_not be_exist(file)
      expect(Oregano::FileSystem).to_not be_exist(symlink)
    end
    ...
    it "should be able to unlink a dangling symlink pointed at a directory" do
      ...
      expect(Oregano::FileSystem).to_not be_exist(dir)
      expect(Oregano::FileSystem).to_not be_exist(symlink)
    end
    ...
    it "should delete only the symlink and not the target when calling unlink instance method" do
      [file, dir].each do |target|
        ...
        expect(Oregano::FileSystem.exist?(target)).to be_truthy
        expect(Oregano::FileSystem.readlink(symlink)).to eq(target.to_s)

        expect(Oregano::FileSystem.unlink(symlink)).to eq(1) # count of files
    ...
    describe "unlink" do
      it "should delete files with unlink" do
        ...
        expect(Oregano::FileSystem.unlink(file)).to eq(1)  # count of files
      ...
      it "should delete multiple files with unlink class method" do
        paths = (1..3).collect do |i|
          ...
          FileUtils.touch(f)
          expect(Oregano::FileSystem.exist?(f)).to be_truthy
          f.to_s
        end

        expect(Oregano::FileSystem.unlink(*paths)).to eq(3)  # count of files

        paths.each { |p| expect(Oregano::FileSystem.exist?(p)).to be_falsey  }
      end
    ...
    describe "exclusive_create" do
      it "should create a file that doesn't exist" do
        ...
        Oregano::FileSystem.exclusive_create(missing_file, nil) {}

        expect(Oregano::FileSystem.exist?(missing_file)).to be_truthy
      end
      ...
      it "should raise Errno::EEXIST creating a file that does exist" do
        ...
        expect do
          Oregano::FileSystem.exclusive_create(file, nil) {}
    ...
    describe 'expand_path' do
      it 'should raise an error when given nil, like Ruby File.expand_path' do
        ...
        # match Ruby behavior
        expect { Oregano::FileSystem.expand_path(nil) }.to raise_error(TypeError)
      end

      it 'with an expanded path passed to Dir.glob, the same expanded path will be returned' do
        # this exists specifically for Oregano::Pops::Loader::ModuleLoaders::FileBased#add_to_index
        ...
        Oregano::FileSystem.touch(tmp_long_file)
        expanded_path = Oregano::FileSystem.expand_path(tmp_long_file)

        expect(expanded_path).to eq(Dir.glob(expanded_path).first)
      end

      describe 'on non-Windows', :unless => Oregano::Util::Platform.windows? do
        it 'should produce the same results as the Ruby File.expand_path' do
          ...
          expect(Oregano::FileSystem.expand_path(path)).to eq(File.expand_path(path))
        end
      end

      describe 'on Windows', :if => Oregano::Util::Platform.windows? do
        ...
        ['/', '\\'].each do |slash|
          it "should return the absolute path including system drive letter when given #{slash}, like Ruby File.expand_path" do
            ...
            expanded = Oregano::FileSystem.expand_path(slash)
        ...
        it 'should behave like Rubys File.expand_path for a file that doesnt exist' do
          expect(Oregano::FileSystem.exist?(nonexist_file)).to be_falsey
          ...
          expect(Oregano::FileSystem.expand_path(nonexist_file)).to eq(ruby_expanded)
        end

        it 'should behave like Rubys File.expand_path for a file with a parent path that doesnt exist' do
          expect(Oregano::FileSystem.exist?(nonexist_path)).to be_falsey
          ...
          expect(ruby_expanded).to match(/~/)
          expect(Oregano::FileSystem.expand_path(nonexist_path)).to eq(ruby_expanded)
        end
        ...
        it 'should expand a shortened path completely, unlike Ruby File.expand_path' do
          ...
          short_path = Oregano::Util::Windows::File.get_short_pathname(tmp_long_dir)
          ...
          # which is the primary reason that a Oregano helper exists
          ruby_expanded = File.expand_path(short_path)
          expect(ruby_expanded).to match(/~/)

          # Oregano expansion uses the Windows API and has no ~ remaining
          oregano_expanded = Oregano::FileSystem.expand_path(short_path)
          expect(oregano_expanded).to_not match(/~/)

          # and the directories are one and the same
          expect(File.identical?(short_path, oregano_expanded)).to be_truthy
        end
      end
    end
  end
