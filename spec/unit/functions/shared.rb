shared_examples_for 'all functions transforming relative to absolute names' do |func_name|
  before(:each) do
    ...
    @resource = Oregano::Parser::Resource.new(:file, "/my/file", :scope => @scope, :source => "source")
  ...
  it 'accepts a Class[name] type' do
    ...
    @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.host_class('myclass')])
  end

  it 'accepts a Resource[class, name] type' do
    @scope.compiler.expects(:evaluate_classes).with(["::myclass"], @scope, false)
    @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.resource('class', 'myclass')])
  end

  it 'raises and error for unspecific Class' do
    expect {
      @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.host_class()])
  ...
  it 'raises and error for Resource that is not of class type' do
    expect {
      @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.resource('file')])
  ...
  it 'raises and error for Resource that is unspecific' do
    expect {
      @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.resource()])
  ...
  it 'raises and error for Resource[class] that is unspecific' do
    expect {
      @scope.call_function(func_name, [Oregano::Pops::Types::TypeFactory.resource('class')])
