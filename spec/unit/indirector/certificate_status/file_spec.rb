...
describe "Oregano::Indirector::CertificateStatus::File" do
  include OreganoSpec::Files

  before :all do
    Oregano::SSL::Host.configure_indirection(:file)
  end

  before do
    Oregano::SSL::CertificateAuthority.stubs(:ca?).returns true
    @terminus = Oregano::SSL::Host.indirection.terminus(:file)

    @tmpdir = tmpdir("certificate_status_ca_testing")
    Oregano[:confdir] = @tmpdir
    Oregano[:vardir] = @tmpdir
  ...
  def generate_csr(host)
    host.generate_key
    csr = Oregano::SSL::CertificateRequest.new(host.name)
  ...
  def sign_csr(host)
    ...
    @terminus.save(Oregano::Indirector::Request.new(:certificate_status, :save, host.name, host))
  end
  ...
  def generate_signed_cert(host)
    ...
    @terminus.find(Oregano::Indirector::Request.new(:certificate_status, :find, host.name, host))
  end
  ...
  it "should be a terminus on SSL::Host" do
    expect(@terminus).to be_instance_of(Oregano::Indirector::CertificateStatus::File)
  end

  it "should create a CA instance if none is present" do
    expect(@terminus.ca).to be_instance_of(Oregano::SSL::CertificateAuthority)
  end

  describe "when creating the CA" do
    it "should fail if it is not a valid CA" do
      Oregano::SSL::CertificateAuthority.expects(:ca?).returns false
  ...
  it "should be indirected with the name 'certificate_status'" do
    expect(Oregano::SSL::Host.indirection.name).to eq(:certificate_status)
  end

  describe "when finding" do
    before do
      @host = Oregano::SSL::Host.new("foo")
      Oregano.settings.use(:main)
    end

    it "should return the Oregano::SSL::Host when a CSR exists for the host" do
      generate_csr(@host)
      request = Oregano::Indirector::Request.new(:certificate_status, :find, "foo", @host)
    ...
    it "should return the Oregano::SSL::Host when a public key exists for the host" do
      generate_signed_cert(@host)
      request = Oregano::Indirector::Request.new(:certificate_status, :find, "foo", @host)
  ...
  describe "when saving" do
    before do
      @host = Oregano::SSL::Host.new("foobar")
      Oregano.settings.use(:main)
    end
    ...
    describe "when signing a cert" do
      before do
        @host.desired_state = "signed"
        @request = Oregano::Indirector::Request.new(:certificate_status, :save, "foobar", @host)
      end

      it "should fail if no CSR is on disk" do
        expect { @terminus.save(@request) }.to raise_error(Oregano::Error, /certificate request/)
      end
      ...
      it "should sign the on-disk CSR when it is present" do
        ...
        expect(Oregano::SSL::Certificate.indirection.find("foobar")).to be_instance_of(Oregano::SSL::Certificate)
      end
    end
    ...
    describe "when revoking a cert" do
      ...
      it "should fail if no certificate is on disk" do
        @host.desired_state = "revoked"
        expect { @terminus.save(@request) }.to raise_error(Oregano::Error, /Cannot revoke/)
      end
  ...
  describe "when deleting" do
    ...
    it "should not delete anything if no certificate, request, or key is on disk" do
      host = Oregano::SSL::Host.new("clean_me")
      request = Oregano::Indirector::Request.new(:certificate_status, :delete, "clean_me", host)
      expect(@terminus.destroy(request)).to eq("Nothing was deleted")
    end

    it "should clean certs, cert requests, keys" do
      signed_host = Oregano::SSL::Host.new("clean_signed_cert")
      generate_signed_cert(signed_host)
      signed_request = Oregano::Indirector::Request.new(:certificate_status, :delete, "clean_signed_cert", signed_host)
      expect(@terminus.destroy(signed_request)).to eq("Deleted for clean_signed_cert: Oregano::SSL::Certificate, Oregano::SSL::Key")

      requested_host = Oregano::SSL::Host.new("clean_csr")
      generate_csr(requested_host)
      csr_request = Oregano::Indirector::Request.new(:certificate_status, :delete, "clean_csr", requested_host)
      expect(@terminus.destroy(csr_request)).to eq("Deleted for clean_csr: Oregano::SSL::CertificateRequest, Oregano::SSL::Key")
    end
  end
  ...
  describe "when searching" do
    it "should return a list of all hosts with certificate requests, signed certs, or revoked certs" do
      ...
      signed_host = Oregano::SSL::Host.new("signed_host")
      generate_signed_cert(signed_host)

      requested_host = Oregano::SSL::Host.new("requested_host")
      generate_csr(requested_host)

      revoked_host = Oregano::SSL::Host.new("revoked_host")
      generate_revoked_cert(revoked_host)

      retrieved_hosts = @terminus.search(Oregano::Indirector::Request.new(:certificate_status, :search, "all", signed_host))
