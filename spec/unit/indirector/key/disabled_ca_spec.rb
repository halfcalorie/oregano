#! /usr/bin/env ruby
require 'spec_helper'
require 'oregano/indirector/key/disabled_ca'

describe Oregano::SSL::Key::DisabledCa do
  def request(type, remote)
    r = Oregano::Indirector::Request.new(:key, type, "foo.com", nil)
    if remote
      r.ip   = '10.0.0.1'
  ...
  context "when not a CA" do
    ...
    [:find, :head, :search, :save, :destroy].each do |name|
      ...
      it "should forward local #{name} requests" do
        Oregano::SSL::Key.indirection.terminus(:file).expects(name)
