...
require 'oregano/indirector/indirector_testing/msgpack'

describe Oregano::Indirector::Msgpack, :if => Oregano.features.msgpack? do
  include OreganoSpec::Files

  subject { Oregano::IndirectorTesting::Msgpack.new }
  ...
  context "#path" do
    ...
    it "uses the :server_datadir setting if this is the master" do
      ...
      expected = File.join(Oregano[:server_datadir], 'indirector_testing', 'testing.msgpack')
    ...
    it "uses the :client_datadir setting if this is not the master" do
      ...
      expected = File.join(Oregano[:client_datadir], 'indirector_testing', 'testing.msgpack')
      expect(subject.path('testing')).to eq(expected)
    end

    it "overrides the default extension with a supplied value" do
      Oregano.run_mode.stubs(:master?).returns(true)
      expected = File.join(Oregano[:server_datadir], 'indirector_testing', 'testing.not-msgpack')
  ...
  context "handling requests" do
    before :each do
      Oregano.run_mode.stubs(:master?).returns(true)
      Oregano[:server_datadir] = tmpdir('msgpackdir')
    ...
    context "#find" do
      ...
      it "raises a descriptive error when the file can't be read" do
        with_content(model.new('foo').to_msgpack) do
          ...
          Oregano::FileSystem.expects(:read).with(file, {:encoding => 'utf-8'}).raises(Errno::EPERM)
          expect { subject.find(request) }.
            to raise_error Oregano::Error, /Could not read MessagePack/
        end
      end
      ...
      it "raises a descriptive error when the file content is invalid" do
        with_content("this is totally invalid MessagePack") do
          expect { subject.find(request) }.
            to raise_error Oregano::Error, /Could not parse MessagePack data/
        end
      end
  ...
  context "#search" do
    ...
    def create_file(name, value = 12)
      File.open(subject.path(name, ''), 'w') do |f|
        f.write Oregano::IndirectorTesting.new(value).to_msgpack
      end
    end
