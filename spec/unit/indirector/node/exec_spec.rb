...
describe Oregano::Node::Exec do
  ...
  describe "when handling the results of the command" do
    let(:testing_env) { Oregano::Node::Environment.create(:testing, []) }
    let(:other_env) { Oregano::Node::Environment.create(:other, []) }
    let(:request) { Oregano::Indirector::Request.new(:node, :find, @name, nil) }
    before do
      @name = "yay"
    ...
    around do |example|
      envs = Oregano::Environments::Static.new(testing_env, other_env)
    ...
    it "should merge facts from the request if supplied" do
      facts = Oregano::Node::Facts.new('test', 'foo' => 'bar')
