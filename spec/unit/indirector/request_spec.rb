...
require 'oregano/indirector/request'

describe Oregano::Indirector::Request do
  include JSONMatchers

  describe "when initializing" do
    it "should always convert the indirection name to a symbol" do
      expect(Oregano::Indirector::Request.new("ind", :method, "mykey", nil).indirection_name).to eq(:ind)
    end

    it "should use provided value as the key if it is a string" do
      expect(Oregano::Indirector::Request.new(:ind, :method, "mykey", nil).key).to eq("mykey")
    end
    ...
    it "should use the name of the provided instance as its key if an instance is provided as the key instead of a string" do
      ...
      request = Oregano::Indirector::Request.new(:ind, :method, nil, instance)
      expect(request.key).to eq("mykey")
      expect(request.instance).to equal(instance)
    end

    it "should support options specified as a hash" do
      expect { Oregano::Indirector::Request.new(:ind, :method, :key, nil, :one => :two) }.to_not raise_error
    end

    it "should support nil options" do
      expect { Oregano::Indirector::Request.new(:ind, :method, :key, nil, nil) }.to_not raise_error
    end

    it "should support unspecified options" do
      expect { Oregano::Indirector::Request.new(:ind, :method, :key, nil) }.to_not raise_error
    end

    it "should use an empty options hash if nil was provided" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, nil).options).to eq({})
    end

    it "should default to a nil node" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil).node).to be_nil
    end

    it "should set its node attribute if provided in the options" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :node => "foo.com").node).to eq("foo.com")
    end

    it "should default to a nil ip" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil).ip).to be_nil
    end

    it "should set its ip attribute if provided in the options" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :ip => "192.168.0.1").ip).to eq("192.168.0.1")
    end

    it "should default to being unauthenticated" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil)).not_to be_authenticated
    end

    it "should set be marked authenticated if configured in the options" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :authenticated => "eh")).to be_authenticated
    end

    it "should keep its options as a hash even if a node is specified" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :node => "eh").options).to be_instance_of(Hash)
    end

    it "should keep its options as a hash even if another option is specified" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :foo => "bar").options).to be_instance_of(Hash)
    end

    it "should treat options other than :ip, :node, and :authenticated as options rather than attributes" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :server => "bar").options[:server]).to eq("bar")
    end

    it "should normalize options to use symbols as keys" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, "foo" => "bar").options[:foo]).to eq("bar")
    end

    describe "and the request key is a URI" do
      let(:file) { File.expand_path("/my/file with spaces") }
      let(:an_environment) { Oregano::Node::Environment.create(:an_environment, []) }
      let(:env_loaders) { Oregano::Environments::Static.new(an_environment) }

      around(:each) do |example|
        Oregano.override({ :environments => env_loaders }, "Static environment loader for specs") do
          example.run
        end
      end
      ...
      describe "and the URI is a 'file' URI" do
        before do
          @request = Oregano::Indirector::Request.new(:ind, :method, "#{URI.unescape(Oregano::Util.path_to_uri(file).to_s)}", nil)
        end
      ...
      it "should set the protocol to the URI scheme" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http://host/", nil).protocol).to eq("http")
      end

      it "should set the server if a server is provided" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http://host/", nil).server).to eq("host")
      end

      it "should set the server and port if both are provided" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http://host:543/", nil).port).to eq(543)
      end

      it "should default to the masterport if the URI scheme is 'oregano'" do
        Oregano[:masterport] = "321"
        expect(Oregano::Indirector::Request.new(:ind, :method, "oregano://host/", nil).port).to eq(321)
      end

      it "should use the provided port if the URI scheme is not 'oregano'" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http://host/", nil).port).to eq(80)
      end

      it "should set the request key to the unescaped path from the URI" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http://host/stuff with spaces", nil).key).to eq("stuff with spaces")
      end

      it "should set the request key to the unescaped path from the URI, in UTF-8 encoding" do
        path = "\u4e07"
        uri = "http://host/#{path}"
        request = Oregano::Indirector::Request.new(:ind, :method, uri, nil)
      ...
      it "should set the request key properly given a UTF-8 URI" do
        ...
        req = Oregano::Indirector::Request.new(:ind, :method, "http:///#{key}", nil)
      ...
      it "should set the :uri attribute to the full URI" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "http:///a/path/stu ff", nil).uri).to eq('http:///a/path/stu ff')
      end

      it "should not parse relative URI" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "foo/bar", nil).uri).to be_nil
      end

      it "should not parse opaque URI" do
        expect(Oregano::Indirector::Request.new(:ind, :method, "mailto:joe", nil).uri).to be_nil
      end
    end

    it "should allow indication that it should not read a cached instance" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :ignore_cache => true)).to be_ignore_cache
    end

    it "should default to not ignoring the cache" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil)).not_to be_ignore_cache
    end

    it "should allow indication that it should not not read an instance from the terminus" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil, :ignore_terminus => true)).to be_ignore_terminus
    end

    it "should default to not ignoring the terminus" do
      expect(Oregano::Indirector::Request.new(:ind, :method, :key, nil)).not_to be_ignore_terminus
    end
  end

  it "should look use the Indirection class to return the appropriate indirection" do
    ind = mock 'indirection'
    Oregano::Indirector::Indirection.expects(:instance).with(:myind).returns ind
    request = Oregano::Indirector::Request.new(:myind, :method, :key, nil)
  ...
  it "should fail intelligently when asked to find a model but the indirection cannot be found" do
    Oregano::Indirector::Indirection.expects(:instance).with(:myind).returns nil
    request = Oregano::Indirector::Request.new(:myind, :method, :key, nil)
  ...
  it "should have a method for determining if the request is plural or singular" do
    expect(Oregano::Indirector::Request.new(:myind, :method, :key, nil)).to respond_to(:plural?)
  end

  it "should be considered plural if the method is 'search'" do
    expect(Oregano::Indirector::Request.new(:myind, :search, :key, nil)).to be_plural
  end

  it "should not be considered plural if the method is not 'search'" do
    expect(Oregano::Indirector::Request.new(:myind, :find, :key, nil)).not_to be_plural
  end

  it "should use its uri, if it has one, as its description" do
    Oregano.override({
      :environments => Oregano::Environments::Static.new(
        Oregano::Node::Environment.create(:baz, [])
    )},
      "Static loader for spec") do
      expect(Oregano::Indirector::Request.new(:myind, :find, "foo://bar/baz", nil).description).to eq("foo://bar/baz")
      end
  end

  it "should use its indirection name and key, if it has no uri, as its description" do
    expect(Oregano::Indirector::Request.new(:myind, :find, "key", nil).description).to eq("/myind/key")
  end
  ...
  it "should set its environment to an environment instance when a string is specified as its environment" do
    ...
    Oregano.override(:environments => Oregano::Environments::Static.new(env)) do
      expect(Oregano::Indirector::Request.new(:myind, :find, "my key", nil, :environment => "foo").environment).to eq(env)
    end
  end
  ...
  it "should use any passed in environment instances as its environment" do
    ...
    expect(Oregano::Indirector::Request.new(:myind, :find, "my key", nil, :environment => env).environment).to equal(env)
  end

  it "should use the current environment when none is provided" do
    configured = Oregano::Node::Environment.create(:foo, [])

    Oregano[:environment] = "foo"

    expect(Oregano::Indirector::Request.new(:myind, :find, "my key", nil).environment).to eq(Oregano.lookup(:current_environment))
  end

  it "should support converting its options to a hash" do
    expect(Oregano::Indirector::Request.new(:myind, :find, "my key", nil )).to respond_to(:to_hash)
  end

  it "should include all of its attributes when its options are converted to a hash" do
    expect(Oregano::Indirector::Request.new(:myind, :find, "my key", nil, :node => 'foo').to_hash[:node]).to eq('foo')
  end

  describe "when building a query string from its options" do
    def a_request_with_options(options)
      Oregano::Indirector::Request.new(:myind, :find, "my key", nil, options)
    end
  ...
  context '#do_request' do
    before :each do
      @request = Oregano::Indirector::Request.new(:myind, :find, "my key", nil)
    end

    context 'when not using SRV records' do
      before :each do
        Oregano.settings[:use_srv_records] = false
      end

      it "yields the request with the default server and port when no server or port were specified on the original request" do
        count = 0
        rval = @request.do_request(:oregano, 'oregano.example.com', '90210') do |got|
          count += 1
          expect(got.server).to eq('oregano.example.com')
    ...
    context 'when using SRV records' do
      before :each do
        Oregano.settings[:use_srv_records] = true
        Oregano.settings[:srv_domain]      = 'example.com'
      end

      it "yields the request with the original server and port unmodified" do
        @request.server = 'oregano.example.com'
        @request.port   = '90210'
      ...
      context "when SRV returns servers" do
        before :each do
          ...
          @port = 7205
          @host = '_x-oregano._tcp.example.com'
          @srv_records = [Resolv::DNS::Resource::IN::SRV.new(0, 0, @port, @host)]

          @dns_mock.expects(:getresources).
            with("_x-oregano._tcp.#{Oregano.settings[:srv_domain]}", Resolv::DNS::Resource::IN::SRV).
            returns(@srv_records)
        end
        ...
        it "yields a request using the server and port from the SRV record" do
          ...
          rval = @request.do_request do |got|
            count += 1
            expect(got.server).to eq('_x-oregano._tcp.example.com')
        ...
        it "should fall back to the default server when the block raises a SystemCallError" do
          ...
          rval = @request.do_request(:oregano, 'oregano', 8140) do |got|
            count += 1

            if got.server == '_x-oregano._tcp.example.com' then
          ...
          expect(second_pass.server).to eq('oregano')
  ...
  describe "#remote?" do
    def request(options = {})
      Oregano::Indirector::Request.new('node', 'find', 'localhost', nil, options)
    end
  ...
  describe "failover" do
    it "should use the provided failover host and port" do
      Oregano.override(:server => 'myhost', :serverport => 666) do
        req = Oregano::Indirector::Request.new('node', 'find', 'localhost', nil)
    ...
    it "should not use raw settings when failover fails" do
      Oregano.override(:server => nil, :serverport => nil) do
        req = Oregano::Indirector::Request.new('node', 'find', 'localhost', nil)
        req.do_request() do |request|
          expect(request.server).to be_nil
          expect(request.port).to be_nil
          expect(Oregano.settings[:server]).not_to be_nil
          expect(Oregano.settings[:masterport]).not_to be_nil
        end
      end
    end

    it "should use server_list when set and failover has not occured" do
      Oregano.settings[:server_list] = [['myhost',666]]
      req = Oregano::Indirector::Request.new('node', 'find', 'localhost', nil)
      req.do_request() do |request|
        expect(request.server).to eq('myhost')
        expect(request.port).to eq(666)
    ...
    it "should use server when server_list is not set" do
      req = Oregano::Indirector::Request.new('node', 'find', 'localhost', nil)
      req.do_request() do |request|
        expect(request.server).to eq(Oregano.settings[:server])
        expect(request.port).to eq(Oregano.settings[:masterport])
      end
    end
  end
end
