...
describe Oregano::Indirector::Yaml do
  ...
  before :all do
    ...
    Oregano::Indirector::Indirection.expects(:instance).with(:my_yaml).returns(@indirection)
    module MyYaml; end
    @store_class = class MyYaml::MyType < Oregano::Indirector::Yaml
      self
    end
  end
  ...
  describe "when choosing file location" do
    ...
    it "should store all files in a single file root set in the Oregano defaults" do
  ...
  describe "when retrieving YAML" do
    it "should read YAML in from disk and convert it to Ruby objects" do
      @store.save(Oregano::Indirector::Request.new(:my_yaml, :save, "testing", @subject))

      expect(@store.find(Oregano::Indirector::Request.new(:my_yaml, :find, "testing", nil)).name).to eq(:me)
    end

    it "should fail coherently when the stored YAML is invalid" do
      saved_structure = Struct.new(:name).new("testing")

      @store.save(Oregano::Indirector::Request.new(:my_yaml, :save, "testing", saved_structure))
      File.open(@store.path(saved_structure.name), "w") do |file|
        file.puts "{ invalid"
      end

      expect {
        @store.find(Oregano::Indirector::Request.new(:my_yaml, :find, "testing", nil))
      }.to raise_error(Oregano::Error, /Could not parse YAML data/)
    end
  end
  ...
  describe "when searching" do
    ...
    describe "when destroying" do
      ...
      it "should not unlink the yaml file if it does not exists" do
        ...
        Oregano::FileSystem.expects(:unlink).with(path).never
