...
describe Oregano::ModuleTool do
  ...
  describe '.find_module_root' do
    ...
    it 'should return the first path as a pathname when it contains a module file' do
      Oregano::ModuleTool.expects(:is_module_root?).with(sample_path).
        returns(true)
    ...
    it 'should return a parent path as a pathname when it contains a module file' do
      Oregano::ModuleTool.expects(:is_module_root?).
    ...
    it 'should return nil when no module root can be found' do
      Oregano::ModuleTool.expects(:is_module_root?).at_least_once.returns(false)
...
TREE
  ...
  describe '.set_option_defaults' do
    ...
    let(:environment) { Oregano::Node::Environment.create(environment_name, modulepath) }
    ...
    around do |example|
      envs = Oregano::Environments::Static.new(environment)
    ...
    describe ':environment' do
      ...
      context 'as Oregano::Node::Environment' do
        let(:env) { Oregano::Node::Environment.create('anonymous', []) }
  ...
  describe '.parse_module_dependency' do
    it 'parses a dependency without a version range expression' do
      ...
      expect(range).to eql(SemanticOregano::VersionRange.parse('>= 0.0.0'))
    ...
    it 'parses a dependency with a version range expression' do
      ...
      expect(name).to eql('foo-bar')
      expect(range).to eql(SemanticOregano::VersionRange.parse('1.2.x'))
    ...
    it 'does not raise an error on invalid version range expressions' do
      ...
      expect(name).to eql('foo-bar')
      expect(range).to eql(SemanticOregano::VersionRange::EMPTY_RANGE)
