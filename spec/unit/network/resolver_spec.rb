...
describe Oregano::Network::Resolver do
  before do
    ...
    @test_a_hostname = "oregano.domain.com"
    @test_port       = 1000

    # The records we should use.
    @test_records = [
      #                                  priority,  weight, port, hostname
      Resolv::DNS::Resource::IN::SRV.new(0,         20,     8140, "oregano1.domain.com"),
      Resolv::DNS::Resource::IN::SRV.new(0,         80,     8140, "oregano2.domain.com"),
      Resolv::DNS::Resource::IN::SRV.new(1,         1,      8140, "oregano3.domain.com"),
      Resolv::DNS::Resource::IN::SRV.new(4,         1,      8140, "oregano4.domain.com")
    ]
  end
  ...
  describe 'when the domain is not known' do
    ...
    describe 'because domain is nil' do
      it 'does not yield' do
        Oregano::Network::Resolver.each_srv_record(nil) do |_,_,_|
    ...
    describe 'because domain is an empty string' do
      it 'does not yield' do
        Oregano::Network::Resolver.each_srv_record('') do |_,_,_|
          raise Exception.new("nil domain caused SRV lookup")
        end
      end
    end
  ...
  describe "when resolving a host without SRV records" do
    it "should not yield anything" do
      ...
      @dns_mock_object.expects(:getresources).with(
        "_x-oregano._tcp.#{@test_a_hostname}",
        @rr_type
      ).returns([])

      Oregano::Network::Resolver.each_srv_record(@test_a_hostname) do |hostname, port, remaining|
  ...
  describe "when resolving a host with SRV records" do
    it "should iterate through records in priority order" do
      ...
      order = {
        0 => ["oregano1.domain.com", "oregano2.domain.com"],
        1 => ["oregano3.domain.com"],
        2 => ["oregano4.domain.com"]
      }

      @dns_mock_object.expects(:getresources).with(
        "_x-oregano._tcp.#{@test_srv_domain}",
        @rr_type
      ).returns(@test_records)

      Oregano::Network::Resolver.each_srv_record(@test_srv_domain) do |hostname, port|
    ...
    it "should fall back to the :oregano service if no records are found for a more specific service" do
      # The order of the records that should be returned,
      # an array means unordered (for weight)
      order = {
        0 => ["oregano1.domain.com", "oregano2.domain.com"],
      ...
      @dns_mock_object.expects(:getresources).with(
        "_x-oregano-report._tcp.#{@test_srv_domain}",
        @rr_type
      ).returns([])
      ...
      Oregano::Network::Resolver.each_srv_record(@test_srv_domain, :report) do |hostname, port|
        expected_priority = order.keys.min
    ...
    it "should use SRV records from the specific service if they exist" do
      ...
      bad_records = [
        #                                  priority,  weight, port, hostname
        Resolv::DNS::Resource::IN::SRV.new(0,         20,     8140, "oregano1.bad.domain.com"),
        Resolv::DNS::Resource::IN::SRV.new(0,         80,     8140, "oregano2.bad.domain.com"),
        Resolv::DNS::Resource::IN::SRV.new(1,         1,      8140, "oregano3.bad.domain.com"),
        Resolv::DNS::Resource::IN::SRV.new(4,         1,      8140, "oregano4.bad.domain.com")
      ]
  ...
  describe "when finding weighted servers" do
    it "should return nil when no records were found" do
      expect(Oregano::Network::Resolver.find_weighted_server([])).to eq(nil)
    end

    it "should return the first record when one record is passed" do
      result = Oregano::Network::Resolver.find_weighted_server([@test_records.first])
...

    {
      ...
      it "should return correct results when #{name}" do
        ...
        total_weight = records.inject(0) do |sum, record|
          sum + Oregano::Network::Resolver.weight(record)
        end

        total_weight.times do |n|
          Kernel.expects(:rand).once.with(total_weight).returns(n)
          server = Oregano::Network::Resolver.find_weighted_server(records)
          seen[server] += 1
        end

        expect(seen.length).to eq(records.length)
        records.each do |record|
          expect(seen[record]).to eq(Oregano::Network::Resolver.weight(record))
        end
      end
    end
  end
