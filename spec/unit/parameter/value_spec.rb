...
describe Oregano::Parameter::Value do
  it "should require a name" do
    expect { Oregano::Parameter::Value.new }.to raise_error(ArgumentError)
  end

  it "should set its name" do
    expect(Oregano::Parameter::Value.new(:foo).name).to eq(:foo)
  end

  it "should support regexes as names" do
    expect { Oregano::Parameter::Value.new(%r{foo}) }.not_to raise_error
  end

  it "should mark itself as a regex if its name is a regex" do
    expect(Oregano::Parameter::Value.new(%r{foo})).to be_regex
  end

  it "should always convert its name to a symbol if it is not a regex" do
    expect(Oregano::Parameter::Value.new("foo").name).to eq(:foo)
    expect(Oregano::Parameter::Value.new(true).name).to eq(:true)
  end

  it "should support adding aliases" do
    expect(Oregano::Parameter::Value.new("foo")).to respond_to(:alias)
  end

  it "should be able to return its aliases" do
    value = Oregano::Parameter::Value.new("foo")
    value.alias("bar")
    value.alias("baz")
  ...
  describe "when matching" do
    describe "a regex" do
      it "should return true if the regex matches the value" do
        expect(Oregano::Parameter::Value.new(/\w/)).to be_match("foo")
      end

      it "should return false if the regex does not match the value" do
        expect(Oregano::Parameter::Value.new(/\d/)).not_to be_match("foo")
      end
    end

    describe "a non-regex" do
      it "should return true if the value, converted to a symbol, matches the name" do
        expect(Oregano::Parameter::Value.new("foo")).to be_match("foo")
        expect(Oregano::Parameter::Value.new(:foo)).to be_match(:foo)
        expect(Oregano::Parameter::Value.new(:foo)).to be_match("foo")
        expect(Oregano::Parameter::Value.new("foo")).to be_match(:foo)
      end
      ...
      it "should return false if the value, converted to a symbol, does not match the name" do
        expect(Oregano::Parameter::Value.new(:foo)).not_to be_match(:bar)
      end
