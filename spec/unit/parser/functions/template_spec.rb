...
describe "the template function" do
  ...
  it "concatenates outputs for multiple templates" do
    ...
    Oregano::Parser::TemplateWrapper.stubs(:new).returns(tw1,tw2)
  ...
  it "raises an error if the template raises an error" do
    ...
    Oregano::Parser::TemplateWrapper.stubs(:new).returns(tw)
    tw.stubs(:result).raises

    expect {
      scope.function_template(["1"])
    }.to raise_error(Oregano::ParseError, /Failed to parse template/)
  end
  ...
  context "when accessing scope variables via method calls (deprecated)" do
    it "raises an error when accessing an undefined variable" do
      ...
      }.to raise_error(Oregano::ParseError, /undefined local variable or method `deprecated'/)
    end
  ...
  it "does not have direct access to Scope#lookupvar" do
    ...
    }.to raise_error(Oregano::ParseError, /undefined method `lookupvar'/)
  end

  def eval_template(content)
    Oregano::FileSystem.stubs(:read_preserve_line_endings).with("template").returns(content)
    Oregano::Parser::Files.stubs(:find_template).returns("template")
