...
describe Oregano::Pops::Model::Factory do
  ...
  context "When factory methods are invoked they should produce expected results" do
    it "tests #var should create a VariableExpression" do
      expect(var('a').model.class).to eq(Oregano::Pops::Model::VariableExpression)
    end

    it "tests #fqn should create a QualifiedName" do
      expect(fqn('a').model.class).to eq(Oregano::Pops::Model::QualifiedName)
    end

    it "tests #QNAME should create a QualifiedName" do
      expect(QNAME('a').model.class).to eq(Oregano::Pops::Model::QualifiedName)
    end

    it "tests #QREF should create a QualifiedReference" do
      expect(QREF('a').model.class).to eq(Oregano::Pops::Model::QualifiedReference)
    end

    it "tests #block should create a BlockExpression" do
      expect(block().model.is_a?(Oregano::Pops::Model::BlockExpression)).to eq(true)
    end

    it "should create a literal undef on :undef" do
      expect(literal(:undef).model.class).to eq(Oregano::Pops::Model::LiteralUndef)
    end

    it "should create a literal default on :default" do
      expect(literal(:default).model.class).to eq(Oregano::Pops::Model::LiteralDefault)
    end
  end

  context "When calling block_or_expression" do
    it "A single expression should produce identical output" do
      expect(block_or_expression([literal(1) + literal(2)]).model.is_a?(Oregano::Pops::Model::ArithmeticExpression)).to eq(true)
    end
    ...
    it "Multiple expressions should produce a block expression" do
      ...
      expect(model.is_a?(Oregano::Pops::Model::BlockExpression)).to eq(true)
  ...
  context "When processing calls with CALL_NAMED" do
    it "Should be possible to state that r-value is required" do
      ...
      expect(built.is_a?(Oregano::Pops::Model::CallNamedFunctionExpression)).to eq(true)
    ...
    it "Should produce a call expression without arguments" do
      ...
      expect(built.is_a?(Oregano::Pops::Model::CallNamedFunctionExpression)).to eq(true)
      expect(built.functor_expr.is_a?(Oregano::Pops::Model::QualifiedName)).to eq(true)
    ...
    it "Should produce a call expression with one argument" do
      ...
      expect(built.arguments[0].is_a?(Oregano::Pops::Model::ArithmeticExpression)).to eq(true)
    end
    ...
    it "Should produce a call expression with two arguments" do
      ...
      expect(built.arguments[0].is_a?(Oregano::Pops::Model::ArithmeticExpression)).to eq(true)
      expect(built.arguments[1].is_a?(Oregano::Pops::Model::ArithmeticExpression)).to eq(true)
    end
  end
  ...
  context "When creating attribute operations" do
    it "Should produce an attribute operation for =>" do
      ...
      built.is_a?(Oregano::Pops::Model::AttributeOperation)
      expect(built.operator).to eq('=>')
      expect(built.attribute_name).to eq("aname")
      expect(built.value_expr.is_a?(Oregano::Pops::Model::LiteralString)).to eq(true)
    end
  ...
  context "When processing RESOURCE" do
    it "Should create a Resource body" do
      ...
      expect(built.is_a?(Oregano::Pops::Model::ResourceBody)).to eq(true)
      expect(built.title.is_a?(Oregano::Pops::Model::LiteralString)).to eq(true)
      expect(built.operations.size).to eq(1)
      expect(built.operations[0].class).to eq(Oregano::Pops::Model::AttributeOperation)
    ...
    it "Should create a RESOURCE without a resource body" do
      ...
      expect(built.class).to eq(Oregano::Pops::Model::ResourceExpression)
  ...
  context "When processing simple literals" do
    it "Should produce a literal boolean from a boolean" do
      ...
      expect(model.class).to eq(Oregano::Pops::Model::LiteralBoolean)
  ...
  context "When processing COLLECT" do
    it "should produce a virtual query" do
      ...
      expect(model.class).to eq(Oregano::Pops::Model::VirtualQuery)
      expect(model.expr.class).to eq(Oregano::Pops::Model::ComparisonExpression)
      expect(model.expr.operator).to eq('==')
    end

    it "should produce an export query" do
      model = EXPORTED_QUERY(fqn('a').eq(literal(1))).model
      expect(model.class).to eq(Oregano::Pops::Model::ExportedQuery)
      expect(model.expr.class).to eq(Oregano::Pops::Model::ComparisonExpression)
      expect(model.expr.operator).to eq('==')
    end
    ...
    it "should produce a collect expression" do
      ...
      expect(built.class).to eq(Oregano::Pops::Model::CollectExpression)
      expect(built.operations.size).to eq(1)
    end
  ...
  context "When processing concatenated string(iterpolation)" do
    it "should handle 'just a string'" do
      ...
      expect(model.class).to eq(Oregano::Pops::Model::ConcatenatedString)
      model.segments.size == 1
      expect(model.segments[0].class).to eq(Oregano::Pops::Model::LiteralString)
    ...
    it "should handle one expression in the middle" do
      ...
      expect(model.segments[0].class).to eq(Oregano::Pops::Model::LiteralString)
      expect(model.segments[0].value).to eq("blah blah")
      expect(model.segments[1].class).to eq(Oregano::Pops::Model::TextExpression)
      expect(model.segments[1].expr.class).to eq(Oregano::Pops::Model::ArithmeticExpression)
      expect(model.segments[2].class).to eq(Oregano::Pops::Model::LiteralString)
    ...
    it "should handle only one expression" do
      ...
      expect(model.class).to eq(Oregano::Pops::Model::ConcatenatedString)
      model.segments.size == 1
      expect(model.segments[0].class).to eq(Oregano::Pops::Model::TextExpression)
      expect(model.segments[0].expr.class).to eq(Oregano::Pops::Model::ArithmeticExpression)
    end
  ...
  context "When processing UNLESS" do
    it "should create an UNLESS expression with then part" do
      ...
      expect(built.class).to eq(Oregano::Pops::Model::UnlessExpression)
      expect(built.test.class).to eq(Oregano::Pops::Model::LiteralBoolean)
      expect(built.then_expr.class).to eq(Oregano::Pops::Model::LiteralInteger)
      expect(built.else_expr.class).to eq(Oregano::Pops::Model::Nop)
    end
    ...
    it "should create an UNLESS expression with then and else parts" do
      ...
      expect(built.test.class).to eq(Oregano::Pops::Model::LiteralBoolean)
      expect(built.then_expr.class).to eq(Oregano::Pops::Model::LiteralInteger)
      expect(built.else_expr.class).to eq(Oregano::Pops::Model::LiteralInteger)
    end
  end
  ...
  context "When processing IF" do
    it "should create an IF expression with then part" do
      ...
      expect(built.class).to eq(Oregano::Pops::Model::IfExpression)
      expect(built.test.class).to eq(Oregano::Pops::Model::LiteralBoolean)
      expect(built.then_expr.class).to eq(Oregano::Pops::Model::LiteralInteger)
      expect(built.else_expr.class).to eq(Oregano::Pops::Model::Nop)
    end
