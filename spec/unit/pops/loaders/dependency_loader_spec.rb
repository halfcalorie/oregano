  ...
  describe 'FileBased module loader' do
    it 'prints a pretty name for itself when inspected' do
      ...
      'lib' => { 'oregano' => { 'functions' => { 'testmodule' => {
        'foo.rb' => 'Oregano::Functions.create_function("foo") { def foo; end; }'
      }}}}})
    ...
    it 'can load something in a qualified name space' do
      module_dir = dir_containing('testmodule', {
      'lib' => { 'oregano' => { 'functions' => { 'testmodule' => {
        'foo.rb' => 'Oregano::Functions.create_function("testmodule::foo") { def foo; end; }'
      }}}}})
    ...
    describe "when parsing files from disk" do
      ...
      let (:node) { Oregano::Node.new('node') }
...
Oregano::Functions.create_function('testmodule::foo') {
  ...
  def loader_for(name, dir)
      module_loader = Oregano::Pops::Loader::ModuleLoaders.module_loader_from(static_loader, loaders, name, dir)
      Oregano::Pops::Loader::DependencyLoader.new(static_loader, 'test-dep', [module_loader])
  end
