...
module Oregano::Pops
module Lookup
describe 'Oregano::Pops::Lookup::Context' do

  context 'an instance' do
    include OreganoSpec::Compiler
    it 'can be created' do
      code = "notice(type(Oregano::LookupContext.new('m')))"
      expect(eval_and_collect_notices(code)[0]).to match(/Object\[\{name => 'Oregano::LookupContext'/)
    end

    it 'returns its environment_name' do
      code = "notice(Oregano::LookupContext.new('m').environment_name)"
      expect(eval_and_collect_notices(code)[0]).to eql('production')
    end

    it 'returns its module_name' do
      code = "notice(Oregano::LookupContext.new('m').module_name)"
      expect(eval_and_collect_notices(code)[0]).to eql('m')
    end

    it 'can use an undef module_name' do
      code = "notice(type(Oregano::LookupContext.new(undef).module_name))"
    ...
    it 'can store and retrieve a value using the cache' do
      code = <<-PUPPET.unindent
        $ctx = Oregano::LookupContext.new('m')
    ...
    context 'with cached_file_data' do
      ...
      let(:populated_env_dir) do
        ...
        OreganoSpec::Files.record_tmp(File.join(env_dir))
        env_dir
      end

      it 'can use cached_file_data without a block' do
        code = <<-PUPPET.unindent
          $ctx = Oregano::LookupContext.new(nil)
      ...
      context 'and multiple compilations' do
        ...
        before(:each) { Oregano.settings[:environment_timeout] = 'unlimited' }
        ...
        it 'will invalidate cache if file changes mtime' do
          old_mtime = Oregano::FileSystem.stat(data_path).mtime
    ...
    context 'when used in an Invocation' do
      let(:node) { Oregano::Node.new('test') }
