...
describe Oregano::Pops::Parser::Parser do
  it "should instantiate a parser" do
    parser = Oregano::Pops::Parser::Parser.new()
    expect(parser.class).to eq(Oregano::Pops::Parser::Parser)
  end
  ...
  it "should parse a code string and return a model" do
    ...
    expect(model.body.class).to eq(Oregano::Pops::Model::AssignmentExpression)
  end
  ...
  it "should accept empty input and return a model" do
    ...
    expect(model.class).to eq(Oregano::Pops::Model::Program)
    expect(model.body.class).to eq(Oregano::Pops::Model::Nop)
  end
...
class firewall {
  if(true) {
    ...
    expect(then_service.class).to eq(Oregano::Pops::Model::ResourceExpression)
    expect(then_service.offset).to eq(34)

    else_service = model.body.body.statements[0].else_expr
    expect(else_service.class).to eq(Oregano::Pops::Model::ResourceExpression)
...
class firewall {
  if(true) {
    ...
    expect(if_expr.class).to eq(Oregano::Pops::Model::IfExpression)
    expect(if_expr.offset).to eq(19)
    expect(block_expr.class).to eq(Oregano::Pops::Model::BlockExpression)
    expect(block_expr.offset).to eq(28)
    expect(block_expr.statements[0].class).to eq(Oregano::Pops::Model::ResourceExpression)
    expect(block_expr.statements[0].offset).to eq(34)
    expect(block_expr.statements[1].class).to eq(Oregano::Pops::Model::ResourceExpression)
...
class firewall {
  unless(true) {
    ...
    if_expr = model.body.body.statements[0]
    block_expr = model.body.body.statements[0].then_expr
    expect(if_expr.class).to eq(Oregano::Pops::Model::UnlessExpression)
    expect(if_expr.offset).to eq(19)
    expect(block_expr.class).to eq(Oregano::Pops::Model::BlockExpression)
  ...
  it "should raise an error with position information when error is raised from within parser" do
    ...
    rescue Oregano::ParseError => e
      the_error = e
    end
    expect(the_error).to be_a(Oregano::ParseError)
