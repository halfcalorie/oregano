...
require 'oregano/provider/command'

describe Oregano::Provider::Command do
  let(:name) { "the name" }
  ...
  it "executes a simple command" do
    ...
    command = Oregano::Provider::Command.new(name, executable, path_resolves_to_itself, executor)
    command.execute()
  end
  ...
  it "executes a command with extra options" do
    ...
    command = Oregano::Provider::Command.new(name, executable, path_resolves_to_itself, executor, the_options)
    command.execute()
  end
  ...
  it "resolves to an absolute path for better execution" do
    ...
    command = Oregano::Provider::Command.new(name, executable, resolver, executor)
    command.execute()
  end
  ...
  it "errors when the executable resolves to nothing" do
    ...
    expect { command.execute() }.to raise_error(Oregano::Error, "Command #{name} is missing")
  end
end
