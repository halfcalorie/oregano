...
provider_class = Oregano::Type.type(:service).provider(:bsd)
...
describe provider_class, :unless => Oregano.features.microsoft_windows? do
  ...
  describe "#disable" do
    ...
    it "should remove a service file to disable" do
      ...
      Oregano::FileSystem.stubs(:exist?).with('/etc/rc.conf.d/sshd').returns(true)
      Oregano::FileSystem.expects(:exist?).with('/etc/rc.conf.d/sshd').returns(true)
    ...
    it "should not remove a service file if it doesn't exist" do
      ...
      Oregano::FileSystem.expects(:exist?).with('/etc/rc.conf.d/sshd').returns(false)
      provider.disable
    end
  end
  ...
  describe "#enabled?" do
    ...
    it "should return false if the service file does not exist" do
      provider = described_class.new(Oregano::Type.type(:service).new(:name => 'sshd'))
      Oregano::FileSystem.stubs(:exist?).with('/etc/rc.conf.d/sshd').returns(false)
      # File.stubs(:read).with('/etc/rc.conf.d/sshd').returns("sshd_enable=\"NO\"\n")
