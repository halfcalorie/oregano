...
provider_class = Oregano::Type.type(:service).provider(:openbsd)
...
describe provider_class, :unless => Oregano.features.microsoft_windows? do
  ...
  describe "#enable" do
    ...
    it "should run rcctl enable with flags if provided" do
      provider = described_class.new(Oregano::Type.type(:service).new(:name => 'sshd', :flags => '-6'))
      described_class.stubs(:rcctl).with(:enable, 'sshd').returns('')
  ...
  describe "#flags" do
    ...
    it "should return flags for special services" do
      provider = described_class.new(Oregano::Type.type(:service).new(:name => 'pf'))
