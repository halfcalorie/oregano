...
describe Oregano::Type.type(:service).provider(:openrc) do
  ...
  before :each do
    ...
    ['/sbin/rc-service', '/bin/rc-status', '/sbin/rc-update'].each do |command|
      # Oregano::Util is both mixed in to providers and is also invoked directly
      # by Oregano::Provider::CommandDefiner, so we have to stub both out.
      described_class.stubs(:which).with(command).returns(command)
      Oregano::Util.stubs(:which).with(command).returns(command)
    end
  end
  ...
  describe 'when invoking `rc-status`' do
    subject { described_class.new(Oregano::Type.type(:service).new(:name => 'urandom')) }
    it "clears the RC_SVCNAME environment variable" do
      Oregano::Util.withenv(:RC_SVCNAME => 'oregano') do
