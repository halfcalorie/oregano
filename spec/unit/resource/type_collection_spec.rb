...
describe Oregano::Resource::TypeCollection do
  ...
  before do
    @instance = Oregano::Resource::Type.new(:hostclass, "foo")
    @code = Oregano::Resource::TypeCollection.new(environment)
  end
  ...
  it "should set itself as the code collection for added resource types" do
    node = Oregano::Resource::Type.new(:node, "foo")
  ...
  it "should fail if a duplicate node is added" do
    @code.add(Oregano::Resource::Type.new(:node, "foo"))

    expect do
      @code.add(Oregano::Resource::Type.new(:node, "foo"))
    end.to raise_error(Oregano::ParseError, /cannot redefine/)
  end

  it "should store hostclasses as hostclasses" do
    klass = Oregano::Resource::Type.new(:hostclass, "foo")
  ...
  it "errors if an attempt is made to merge hostclasses of the same name" do
    klass1 = Oregano::Resource::Type.new(:hostclass, "foo", :doc => "first")
    klass2 = Oregano::Resource::Type.new(:hostclass, "foo", :doc => "second")
  ...
  it "should store definitions as definitions" do
    define = Oregano::Resource::Type.new(:definition, "foo")
  ...
  it "should fail if a duplicate definition is added" do
    @code.add(Oregano::Resource::Type.new(:definition, "foo"))

    expect do
      @code.add(Oregano::Resource::Type.new(:definition, "foo"))
    end.to raise_error(Oregano::ParseError, /cannot be redefined/)
  end

  it "should remove all nodes, classes, definitions, and applications when cleared" do
    loader = Oregano::Resource::TypeCollection.new(environment)
    loader.add Oregano::Resource::Type.new(:hostclass, "class")
    loader.add Oregano::Resource::Type.new(:definition, "define")
    loader.add Oregano::Resource::Type.new(:node, "node")
    loader.add Oregano::Resource::Type.new(:application, "application")
  ...
  describe "when looking up names" do
    before do
      @type = Oregano::Resource::Type.new(:hostclass, "ns::klass")
    end
    ...
    describe "that need to be loaded" do
      ...
      it "already-loaded names at broader scopes should not shadow autoloaded names" do
        @code.add Oregano::Resource::Type.new(:hostclass, "bar")
      ...
      context 'when debugging' do
        ...
        it "should not try to autoload names that we couldn't autoload in a previous step if ignoremissingtypes is enabled" do
          Oregano[:ignoremissingtypes] = true
          @code.loader.expects(:try_load_fqname).with(:hostclass, "ns::klass").returns(nil)
          expect(@code.find_hostclass("ns::klass")).to be_nil
          Oregano.expects(:debug).at_least_once.with {|msg| msg =~ /Not attempting to load hostclass/}
          expect(@code.find_hostclass("ns::klass")).to be_nil
        end
      end
    end
  ...
  KINDS.each do |data|
    describe "behavior of add for #{data}" do
      ...
      it "should return the added #{data}" do
        loader = Oregano::Resource::TypeCollection.new(environment)
        instance = Oregano::Resource::Type.new(data, "foo")
      ...
      it "should retrieve #{data} insensitive to case" do
        loader = Oregano::Resource::TypeCollection.new(environment)
        instance = Oregano::Resource::Type.new(data, "Bar")
      ...
      it "should return nil when asked for a #{data} that has not been added" do
        expect(Oregano::Resource::TypeCollection.new(environment).send(data, "foo")).to be_nil
      end
      ...
      if data != "node"
        it "should fail if an application with the same name is added" do
          loader = Oregano::Resource::TypeCollection.new(environment)
          instance = Oregano::Resource::Type.new(data, "foo")
          application = Oregano::Resource::Type.new(:application, "foo")
          loader.add(instance)
          expect { loader.add(application) }.to raise_error(Oregano::ParseError, /redefine/)
        end
  ...
  describe "when finding a qualified instance" do
    it "should return any found instance if the instance name is fully qualified" do
      loader = Oregano::Resource::TypeCollection.new(environment)
      instance = Oregano::Resource::Type.new(:hostclass, "foo::bar")
      loader.add instance
    ...
    it "should return nil if the object cannot be found" do
      loader = Oregano::Resource::TypeCollection.new(environment)
      instance = Oregano::Resource::Type.new(:hostclass, "foo::bar::baz")
      loader.add instance
    ...
    describe "when topscope has a class that has the same name as a local class" do
      before do
        ...
        [ "foo::bar", "bar" ].each do |name|
          @loader.add Oregano::Resource::Type.new(:hostclass, name)
        end
      end
    ...
    it "should not look in the local scope for classes when the name is qualified" do
        @loader = Oregano::Resource::TypeCollection.new(environment)
        @loader.add Oregano::Resource::Type.new(:hostclass, "foo::bar")
  ...
  it "should be able to find nodes" do
    node = Oregano::Resource::Type.new(:node, "bar")
    loader = Oregano::Resource::TypeCollection.new(environment)
    loader.add(node)
  ...
  it "should indicate whether any nodes are defined" do
    loader = Oregano::Resource::TypeCollection.new(environment)
    loader.add_node(Oregano::Resource::Type.new(:node, "foo"))
    expect(loader).to be_nodes
  end

  it "should indicate whether no nodes are defined" do
    expect(Oregano::Resource::TypeCollection.new(environment)).not_to be_nodes
  end
  ...
  describe "when finding nodes" do
    ...
    it "should return the first regex node whose regex matches the provided node name" do
      node1 = Oregano::Resource::Type.new(:node, /\w/)
      node2 = Oregano::Resource::Type.new(:node, /\d/)
      @loader << node1 << node2
    ...
    it "should preferentially return a node whose name is string-equal over returning a node whose regex matches a provided name" do
      node1 = Oregano::Resource::Type.new(:node, /\w/)
      node2 = Oregano::Resource::Type.new(:node, "foo")
      @loader << node1 << node2
  ...
  describe "when determining the configuration version" do
    ...
    context "when config_version script is specified" do
      let(:environment) { Oregano::Node::Environment.create(:testing, [], '', '/my/foo') }

      it "should use the output of the environment's config_version setting if one is provided" do
        Oregano::Util::Execution.expects(:execute).with(["/my/foo"]).returns "output\n"
        expect(@code.version).to eq("output")
      end

      it "should raise a oregano parser error if executing config_version fails" do
        Oregano::Util::Execution.expects(:execute).raises(Oregano::ExecutionFailure.new("msg"))

        expect { @code.version }.to raise_error(Oregano::ParseError)
      end
    end
  end
end
