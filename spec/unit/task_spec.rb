...
require 'oregano/module/task'

describe Oregano::Module::Task do
  ...
  it "cannot construct tasks with illegal names" do
    expect { Oregano::Module::Task.new(mymod, "iLegal", []) }
      .to raise_error(Oregano::Module::Task::InvalidName,
  ...
  it "cannot construct tasks whose files are outside of their module's tasks directory" do
    ...
    expect { Oregano::Module::Task.new(mymod, "fail", [outside_file]) }
      .to raise_error(Oregano::Module::Task::InvalidFile,
  ...
  it "constructs tasks as expected when every task has a metadata file with the same name (besides extension)" do
    ...
    tasks = Oregano::Module::Task.tasks_in_module(mymod)
  ...
  it "gives the 'init' task a name that is just the module's name" do
    expect(Oregano::Module::Task.new(mymod, 'init', ["#{tasks_path}/init.sh"]).name).to eq('mymod')
  end
end
