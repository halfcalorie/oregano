...
describe Oregano::Type.type(:service) do
  it "should have an :enableable feature that requires the :enable, :disable, and :enabled? methods" do
    expect(Oregano::Type.type(:service).provider_feature(:enableable).methods).to eq([:disable, :enable, :enabled?])
  end

  it "should have a :refreshable feature that requires the :restart method" do
    expect(Oregano::Type.type(:service).provider_feature(:refreshable).methods).to eq([:restart])
  end
end

describe Oregano::Type.type(:service), "when validating attributes" do
  [:name, :binary, :hasstatus, :path, :pattern, :start, :restart, :stop, :status, :hasrestart, :control].each do |param|
    it "should have a #{param} parameter" do
      expect(Oregano::Type.type(:service).attrtype(param)).to eq(:param)
    end
  end

  [:ensure, :enable].each do |param|
    it "should have an #{param} property" do
      expect(Oregano::Type.type(:service).attrtype(param)).to eq(:property)
    end
  end
end

describe Oregano::Type.type(:service), "when validating attribute values" do
  before do
    @provider = stub 'provider', :class => Oregano::Type.type(:service).defaultprovider, :clear => nil, :controllable? => false
    Oregano::Type.type(:service).defaultprovider.stubs(:new).returns(@provider)
  end

  it "should support :running as a value to :ensure" do
    Oregano::Type.type(:service).new(:name => "yay", :ensure => :running)
  end

  it "should support :stopped as a value to :ensure" do
    Oregano::Type.type(:service).new(:name => "yay", :ensure => :stopped)
  end

  it "should alias the value :true to :running in :ensure" do
    svc = Oregano::Type.type(:service).new(:name => "yay", :ensure => true)
    expect(svc.should(:ensure)).to eq(:running)
  end

  it "should alias the value :false to :stopped in :ensure" do
    svc = Oregano::Type.type(:service).new(:name => "yay", :ensure => false)
  ...
  describe "the enable property" do
    ...
    it "should support :true as a value" do
      srv = Oregano::Type.type(:service).new(:name => "yay", :enable => :true)
      expect(srv.should(:enable)).to eq(:true)
    end

    it "should support :false as a value" do
      srv = Oregano::Type.type(:service).new(:name => "yay", :enable => :false)
      expect(srv.should(:enable)).to eq(:false)
    end

    it "should support :mask as a value" do
      srv = Oregano::Type.type(:service).new(:name => "yay", :enable => :mask)
    ...
    it "should support :manual as a value on Windows" do
      ...
      srv = Oregano::Type.type(:service).new(:name => "yay", :enable => :manual)
    ...
    it "should not support :manual as a value when not on Windows" do
      ...
      expect { Oregano::Type.type(:service).new(:name => "yay", :enable => :manual) }.to raise_error(
        Oregano::Error,
  ...
  it "should allow setting the :enable parameter if the provider has the :enableable feature" do
    Oregano::Type.type(:service).defaultprovider.stubs(:supports_parameter?).returns(true)
    Oregano::Type.type(:service).defaultprovider.expects(:supports_parameter?).with(Oregano::Type.type(:service).attrclass(:enable)).returns(true)
    svc = Oregano::Type.type(:service).new(:name => "yay", :enable => true)
  ...
  it "should not allow setting the :enable parameter if the provider is missing the :enableable feature" do
    Oregano::Type.type(:service).defaultprovider.stubs(:supports_parameter?).returns(true)
    Oregano::Type.type(:service).defaultprovider.expects(:supports_parameter?).with(Oregano::Type.type(:service).attrclass(:enable)).returns(false)
    svc = Oregano::Type.type(:service).new(:name => "yay", :enable => true)
  ...
  it "should split paths on '#{File::PATH_SEPARATOR}'" do
    ...
    svc = Oregano::Type.type(:service).new(:name => "yay", :path => "/one/two#{File::PATH_SEPARATOR}/three/four")
  ...
  it "should accept arrays of paths joined by '#{File::PATH_SEPARATOR}'" do
    Oregano::FileSystem.stubs(:exist?).returns(true)
    FileTest.stubs(:directory?).returns(true)
    svc = Oregano::Type.type(:service).new(:name => "yay", :path => ["/one#{File::PATH_SEPARATOR}/two", "/three#{File::PATH_SEPARATOR}/four"])
    expect(svc[:path]).to eq(%w{/one /two /three /four})
  end
end

describe Oregano::Type.type(:service), "when setting default attribute values" do
  it "should default to the provider's default path if one is available" do
    FileTest.stubs(:directory?).returns(true)
    Oregano::FileSystem.stubs(:exist?).returns(true)

    Oregano::Type.type(:service).defaultprovider.stubs(:respond_to?).returns(true)
    Oregano::Type.type(:service).defaultprovider.stubs(:defpath).returns("testing")
    svc = Oregano::Type.type(:service).new(:name => "other")
    expect(svc[:path]).to eq(["testing"])
  end

  it "should default 'pattern' to the binary if one is provided" do
    svc = Oregano::Type.type(:service).new(:name => "other", :binary => "/some/binary")
  ...
  it "should default 'control' to the upcased service name with periods replaced by underscores if the provider supports the 'controllable' feature" do
    provider = stub 'provider', :controllable? => true, :class => Oregano::Type.type(:service).defaultprovider, :clear => nil
    Oregano::Type.type(:service).defaultprovider.stubs(:new).returns(provider)
    svc = Oregano::Type.type(:service).new(:name => "nfs.client")
    expect(svc[:control]).to eq("NFS_CLIENT_START")
  end
end

describe Oregano::Type.type(:service), "when retrieving the host's current state" do
  before do
    @service = Oregano::Type.type(:service).new(:name => "yay")
  end
...
describe Oregano::Type.type(:service), "when changing the host" do
  before do
    @service = Oregano::Type.type(:service).new(:name => "yay")
  end
  ...
  it "should always consider the enable state of a static service to be in sync" do
    ...
    @service[:enable] = false
    Oregano.expects(:debug).with("Unable to enable or disable static service yay")
...
describe Oregano::Type.type(:service), "when refreshing the service" do
  before do
    @service = Oregano::Type.type(:service).new(:name => "yay")
  end
