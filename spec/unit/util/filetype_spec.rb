...
require 'oregano/util/filetype'

# XXX Import all of the tests into this file.
describe Oregano::Util::FileType do
  describe "the flat filetype" do
    let(:path) { '/my/file' }
    let(:type) { Oregano::Util::FileType.filetype(:flat) }
    ...
    describe "when the file already exists" do
      it "should return the file's contents when asked to read it" do
        ...
        Oregano::FileSystem.expects(:read).with(path, :encoding => Encoding.default_external).returns "my text"
    ...
    describe "when writing the file" do
      ...
      it "should first create a temp file and copy its contents over to the file location" do
        Tempfile.expects(:new).with("oregano", :encoding => Encoding.default_external).returns tempfile
    ...
    describe "when backing up a file" do
      ...
      it "should use the default filebucket" do
        ...
        Oregano::Type.type(:filebucket).expects(:mkdefaultbucket).returns bucket
  ...
  shared_examples_for "crontab provider" do
    ...
    # make Oregano::Util::SUIDManager return something deterministic, not the
    # uid of the user running the tests, except where overridden below.
    before :each do
      Oregano::Util::SUIDManager.stubs(:uid).returns 1234
    end

    describe "#read" do
      it "should run crontab -l as the target user" do
        Oregano::Util::Execution.expects(:execute).with(['crontab', '-l'], user_options).returns crontab
        expect(cron.read).to eq(crontab)
      end

      it "should not switch user if current user is the target user" do
        Oregano::Util.expects(:uid).with(uid).returns 9000
        Oregano::Util::SUIDManager.expects(:uid).returns 9000
        Oregano::Util::Execution.expects(:execute).with(['crontab', '-l'], options).returns crontab
        expect(cron.read).to eq(crontab)
      end

      it "should treat an absent crontab as empty" do
        Oregano::Util::Execution.expects(:execute).with(['crontab', '-l'], user_options).raises(Oregano::ExecutionFailure, absent_crontab)
        expect(cron.read).to eq('')
      end

      it "should raise an error if the user is not authorized to use cron" do
        Oregano::Util::Execution.expects(:execute).with(['crontab', '-l'], user_options).raises(Oregano::ExecutionFailure, unauthorized_crontab)
        expect {
          cron.read
        }.to raise_error Oregano::Error, /User #{uid} not authorized to use cron/
      end
    end

    describe "#remove" do
      it "should run crontab -r as the target user" do
        Oregano::Util::Execution.expects(:execute).with(['crontab', '-r'], user_options)
        cron.remove
      end
      ...
      it "should not switch user if current user is the target user" do
        Oregano::Util.expects(:uid).with(uid).returns 9000
        Oregano::Util::SUIDManager.expects(:uid).returns 9000
        Oregano::Util::Execution.expects(:execute).with(['crontab','-r'], options)
        cron.remove
      end
    end
    ...
    describe "#write" do
      before :each do
        @tmp_cron = Tempfile.new("oregano_crontab_spec")
        @tmp_cron_path = @tmp_cron.path
        Oregano::Util.stubs(:uid).with(uid).returns 9000
        Tempfile.expects(:new).with("oregano_#{name}", :encoding => Encoding.default_external).returns @tmp_cron
      end

      after :each do
        expect(Oregano::FileSystem.exist?(@tmp_cron_path)).to be_falsey
      end

      it "should run crontab as the target user on a temporary file" do
        File.expects(:chown).with(9000, nil, @tmp_cron_path)
        Oregano::Util::Execution.expects(:execute).with(["crontab", @tmp_cron_path], user_options)
      ...
      it "should not switch user if current user is the target user" do
        Oregano::Util::SUIDManager.expects(:uid).returns 9000
        File.expects(:chown).with(9000, nil, @tmp_cron_path)
        Oregano::Util::Execution.expects(:execute).with(["crontab", @tmp_cron_path], options)
  ...
  describe "the suntab filetype", :unless => Oregano.features.microsoft_windows? do
    let(:type)           { Oregano::Util::FileType.filetype(:suntab) }
    let(:name)           { type.name }
  ...
  describe "the aixtab filetype", :unless => Oregano.features.microsoft_windows? do
    let(:type)           { Oregano::Util::FileType.filetype(:aixtab) }
    let(:name)           { type.name }
