require 'spec_helper'
require 'oregano/util/plist'
require 'oregano_spec/files'

describe Oregano::Util::Plist, :if => Oregano.features.cfpropertylist? do
     ...
     <dict>
       ...
       <array>
         <dict>
                 ...
                 <key>PrinterID</key>
                 <string>baskerville_corp_oreganolabs_net</string>
         </dict>
         <dict>
                 <key>Network</key>
  ...
  let(:valid_xml_plist_hash) { {"LastUsedPrinters"=>[{"Network"=>"10.85.132.1", "PrinterID"=>"baskerville_corp_oreganolabs_net"}, {"Network"=>"10.14.96.1", "PrinterID"=>"Statler"}]} }
  ...
  describe "#read_plist_file" do
    ...
    it "raises a debug message and replaces a bad XML plist doctype should one be encountered" do
      ...
      Oregano.expects(:debug).with("Had to fix plist with incorrect DOCTYPE declaration: #{plist_path}")
      expect(subject.read_plist_file(plist_path)).to eq('plist_hash')
    end
    it "attempts to read pure xml using plutil when reading an improperly formatted service plist" do
      ...
      Oregano.expects(:debug).with(regexp_matches(/^Failed with CFFormatError/))
      Oregano.expects(:debug).with("Plist #{plist_path} ill-formatted, converting with plutil")
      Oregano::Util::Execution.expects(:execute).with(['/usr/bin/plutil', '-convert', 'xml1', '-o', '/dev/stdout', plist_path],
    ...
    it "returns nil when direct parsing and plutil conversion both fail" do
      ...
      Oregano.expects(:debug).with(regexp_matches(/^Failed with (CFFormatError|NoMethodError)/))
      Oregano.expects(:debug).with("Plist #{plist_path} ill-formatted, converting with plutil")
      Oregano::Util::Execution.expects(:execute).with(['/usr/bin/plutil', '-convert', 'xml1', '-o', '/dev/stdout', plist_path],
                                                     {:failonfail => true, :combine => true}).raises(Oregano::ExecutionFailure, 'boom')
    ...
    it "returns nil when file is a non-plist binary blob" do
      ...
      Oregano.expects(:debug).with(regexp_matches(/^Failed with (CFFormatError|ArgumentError)/))
      Oregano.expects(:debug).with("Plist #{plist_path} ill-formatted, converting with plutil")
      Oregano::Util::Execution.expects(:execute).with(['/usr/bin/plutil', '-convert', 'xml1', '-o', '/dev/stdout', plist_path],
                                                     {:failonfail => true, :combine => true}).raises(Oregano::ExecutionFailure, 'boom')
      expect(subject.read_plist_file(plist_path)).to eq(nil)
