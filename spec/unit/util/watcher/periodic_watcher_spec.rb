...
describe Oregano::Util::Watcher::PeriodicWatcher do
  ...
  it 'reads only the initial change state when the timeout has not yet expired' do
    watcher = Oregano::Util::Watcher::PeriodicWatcher.new(unchanged_watcher, an_unexpired_timer(enabled_timeout))

    expect(watcher).to_not be_changed
  end

  it 'reads enough values to determine change when the timeout has expired' do
    watcher = Oregano::Util::Watcher::PeriodicWatcher.new(changed_watcher, an_expired_timer(enabled_timeout))

    expect(watcher).to be_changed
  end

  it 'is always marked as changed when the timeout is disabled' do
    watcher = Oregano::Util::Watcher::PeriodicWatcher.new(unused_watcher, an_expired_timer(disabled_timeout))

    expect(watcher).to be_changed
  end

  def a_watcher_reporting(*observed_values)
    Oregano::Util::Watcher::ChangeWatcher.watch(proc do
  ...
  def a_time_that_reports_expired_as(expired, timeout)
    timer = Oregano::Util::Watcher::Timer.new(timeout)
