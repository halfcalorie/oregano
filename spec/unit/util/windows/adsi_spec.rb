...
describe Oregano::Util::Windows::ADSI, :if => Oregano.features.microsoft_windows? do
  ...
  let(:ntauthority_localized) { Oregano::Util::Windows::SID::Principal.lookup_account_name('SYSTEM').domain }

  before(:each) do
    Oregano::Util::Windows::ADSI.instance_variable_set(:@computer_name, 'testcomputername')
    Oregano::Util::Windows::ADSI.stubs(:connect).returns connection
  end

  after(:each) do
    Oregano::Util::Windows::ADSI.instance_variable_set(:@computer_name, nil)
  end

  it "should generate the correct URI for a resource" do
    expect(Oregano::Util::Windows::ADSI.uri('test', 'user')).to eq("WinNT://./test,user")
  end

  it "should be able to get the name of the computer" do
    expect(Oregano::Util::Windows::ADSI.computer_name).to eq('testcomputername')
  end

  it "should be able to provide the correct WinNT base URI for the computer" do
    expect(Oregano::Util::Windows::ADSI.computer_uri).to eq("WinNT://.")
  end

  it "should generate a fully qualified WinNT URI" do
    expect(Oregano::Util::Windows::ADSI.computer_uri('testcomputername')).to eq("WinNT://testcomputername")
  end

  describe ".computer_name" do
    it "should return a non-empty ComputerName string" do
      Oregano::Util::Windows::ADSI.instance_variable_set(:@computer_name, nil)
      expect(Oregano::Util::Windows::ADSI.computer_name).not_to be_empty
    end
  end
  ...
  describe ".sid_uri" do
    it "should raise an error when the input is not a SID Principal" do
      [Object.new, {}, 1, :symbol, '', nil].each do |input|
        expect {
          Oregano::Util::Windows::ADSI.sid_uri(input)
        }.to raise_error(Oregano::Error, /Must use a valid SID::Principal/)
      end
    end

    it "should return a SID uri for a well-known SID (SYSTEM)" do
      sid = Oregano::Util::Windows::SID::Principal.lookup_account_name('SYSTEM')
      expect(Oregano::Util::Windows::ADSI.sid_uri(sid)).to eq('WinNT://S-1-5-18')
    end
  end

  describe Oregano::Util::Windows::ADSI::User do
    let(:username)  { 'testuser' }
    let(:domain)    { 'DOMAIN' }
    let(:domain_username) { "#{domain}\\#{username}"}

    it "should generate the correct URI" do
      expect(Oregano::Util::Windows::ADSI::User.uri(username)).to eq("WinNT://./#{username},user")
    end

    it "should generate the correct URI for a user with a domain" do
      expect(Oregano::Util::Windows::ADSI::User.uri(username, domain)).to eq("WinNT://#{domain}/#{username},user")
    end

    it "should generate the correct URI for a BUILTIN user" do
      expect(Oregano::Util::Windows::ADSI::User.uri(username, builtin_localized)).to eq("WinNT://./#{username},user")
    end

    it "should generate the correct URI for a NT AUTHORITY user" do
      expect(Oregano::Util::Windows::ADSI::User.uri(username, ntauthority_localized)).to eq("WinNT://./#{username},user")
    end

    it "should be able to parse a username without a domain" do
      expect(Oregano::Util::Windows::ADSI::User.parse_name(username)).to eq([username, '.'])
    end

    it "should be able to parse a username with a domain" do
      expect(Oregano::Util::Windows::ADSI::User.parse_name(domain_username)).to eq([username, domain])
    end

    it "should raise an error with a username that contains a /" do
      expect {
        Oregano::Util::Windows::ADSI::User.parse_name("#{domain}/#{username}")
      }.to raise_error(Oregano::Error, /Value must be in DOMAIN\\user style syntax/)
    end
    ...
    it "should be able to create a user" do
      ...
      Oregano::Util::Windows::ADSI::Group.expects(:exists?).with(username).returns(false)

      user = Oregano::Util::Windows::ADSI::User.create(username)

      expect(user).to be_a(Oregano::Util::Windows::ADSI::User)
      expect(user.native_user).to eq(adsi_user)
    end

    it "should be able to check the existence of a user" do
      Oregano::Util::Windows::SID.expects(:name_to_sid_object).with(username).returns nil
      Oregano::Util::Windows::ADSI.expects(:connect).with("WinNT://./#{username},user").returns connection
      connection.expects(:Class).returns('User')
      expect(Oregano::Util::Windows::ADSI::User.exists?(username)).to be_truthy
    end

    it "should be able to check the existence of a domain user" do
      Oregano::Util::Windows::SID.expects(:name_to_sid_object).with("#{domain}\\#{username}").returns nil
      Oregano::Util::Windows::ADSI.expects(:connect).with("WinNT://#{domain}/#{username},user").returns connection
      connection.expects(:Class).returns('User')
      expect(Oregano::Util::Windows::ADSI::User.exists?(domain_username)).to be_truthy
    end

    it "should be able to confirm the existence of a user with a well-known SID" do
      system_user = Oregano::Util::Windows::SID::LocalSystem
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::User.exists?(system_user)).to be_truthy
    end

    it "should return false with a well-known Group SID" do
      group = Oregano::Util::Windows::SID::BuiltinAdministrators
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::User.exists?(group)).to be_falsey
    end
    ...
    it "should return nil with an unknown SID" do
      ...
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::User.exists?(bogus_sid)).to be_falsey
    end
    ...
    it "should return an enumeration of IADsUser wrapped objects" do
      ...
      Oregano::Util::Windows::ADSI.expects(:execquery).with('select name from win32_useraccount where localaccount = "TRUE"').returns(wmi_users)

      native_user = stub('IADsUser')
      homedir = "C:\\Users\\#{name}"
      native_user.expects(:Get).with('HomeDirectory').returns(homedir)
      Oregano::Util::Windows::ADSI.expects(:connect).with("WinNT://./#{name},user").returns(native_user)

      users = Oregano::Util::Windows::ADSI::User.to_a
    ...
    describe "an instance" do
      ...
      let(:user)      { Oregano::Util::Windows::ADSI::User.new(username, adsi_user) }
      ...
      it "should be able to test whether a given password is correct" do
        Oregano::Util::Windows::ADSI::User.expects(:logon).with(username, 'pwdwrong').returns(false)
        Oregano::Util::Windows::ADSI::User.expects(:logon).with(username, 'pwdright').returns(true)
      ...
      it "should generate the correct URI" do
        Oregano::Util::Windows::SID.stubs(:octet_string_to_sid_object).returns(sid)
      ...
      describe "when given a set of groups to which to add the user" do
        ...
        describe "if membership is specified as inclusive" do
          it "should add the user to those groups, and remove it from groups not in the list" do
            Oregano::Util::Windows::ADSI::User.expects(:name_sid_hash).returns(Hash[ desired_sids.map { |s| [s.objectSID.sid, s.objectSID] }])
          ...
          it "should remove all users from a group if desired is empty" do
            Oregano::Util::Windows::ADSI::User.expects(:name_sid_hash).returns({})
  ...
  describe Oregano::Util::Windows::ADSI::Group do
    let(:groupname)  { 'testgroup' }

    describe "an instance" do
      let(:adsi_group) { stub 'group' }
      let(:group)      { Oregano::Util::Windows::ADSI::Group.new(groupname, adsi_group) }
      let(:someone_sid){ stub(:account => 'someone', :domain => 'testcomputername')}

      describe "should be able to use SID objects" do
        let(:system)     { Oregano::Util::Windows::SID.name_to_sid_object('SYSTEM') }
        let(:invalid)    { Oregano::Util::Windows::SID.name_to_sid_object('foobar') }
        ...
        it "and raise when passed a non-SID object to add" do
          expect{ group.add_member_sids(invalid)}.to raise_error(Oregano::Error, /Must use a valid SID::Principal/)
        end
        ...
        it "and raise when passed a non-SID object to remove" do
          expect{ group.remove_member_sids(invalid)}.to raise_error(Oregano::Error, /Must use a valid SID::Principal/)
        end
      end
      ...
      context "calling .set_members" do
        it "should set the members of a group to only desired_members when inclusive" do
          ...
          Oregano::Util::Windows::SID.expects(:octet_string_to_sid_object).with([0]).returns(sids[0])
          Oregano::Util::Windows::SID.expects(:octet_string_to_sid_object).with([1]).returns(sids[1])

          Oregano::Util::Windows::SID.expects(:name_to_sid_object).with('user2').returns(sids[1])
          Oregano::Util::Windows::SID.expects(:name_to_sid_object).with('DOMAIN2\user3').returns(sids[2])

          Oregano::Util::Windows::ADSI.expects(:sid_uri).with(sids[0]).returns("WinNT://DOMAIN/user1,user")
          Oregano::Util::Windows::ADSI.expects(:sid_uri).with(sids[2]).returns("WinNT://DOMAIN2/user3,user")
        ...
        it "should remove all members when desired_members is empty and inclusive" do
          ...
          Oregano::Util::Windows::ADSI.expects(:sid_uri).with(sids[0]).returns("WinNT://DOMAIN/user1,user")
          Oregano::Util::Windows::ADSI.expects(:sid_uri).with(sids[1]).returns("WinNT://testcomputername/user2,user")
        ...
        it "should raise an error when a username does not resolve to a SID" do
          ...
          }.to raise_error(Oregano::Error, /Could not resolve name: foobar/)
        end
      end
      ...
      it "should generate the correct URI" do
        ...
        Oregano::Util::Windows::SID.expects(:octet_string_to_sid_object).with([0]).returns(computer_sid)
        expect(group.uri).to eq("WinNT://./#{groupname},group")
      end
    end

    it "should generate the correct URI" do
      expect(Oregano::Util::Windows::ADSI::Group.uri("people")).to eq("WinNT://./people,group")
    end

    it "should generate the correct URI for a BUILTIN group" do
      expect(Oregano::Util::Windows::ADSI::Group.uri(groupname, builtin_localized)).to eq("WinNT://./#{groupname},group")
    end

    it "should generate the correct URI for a NT AUTHORITY group" do
      expect(Oregano::Util::Windows::ADSI::Group.uri(groupname, ntauthority_localized)).to eq("WinNT://./#{groupname},group")
    end
    ...
    it "should be able to create a group" do
      ...
      Oregano::Util::Windows::ADSI::User.expects(:exists?).with(groupname).returns(false)

      group = Oregano::Util::Windows::ADSI::Group.create(groupname)

      expect(group).to be_a(Oregano::Util::Windows::ADSI::Group)
      expect(group.native_group).to eq(adsi_group)
    end

    it "should be able to confirm the existence of a group" do
      Oregano::Util::Windows::SID.expects(:name_to_sid_object).with(groupname).returns nil
      Oregano::Util::Windows::ADSI.expects(:connect).with("WinNT://./#{groupname},group").returns connection
      connection.expects(:Class).returns('Group')

      expect(Oregano::Util::Windows::ADSI::Group.exists?(groupname)).to be_truthy
    end

    it "should be able to confirm the existence of a group with a well-known SID" do

      service_group = Oregano::Util::Windows::SID::Service
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::Group.exists?(service_group)).to be_truthy
    end

    it "will return true with a well-known User SID, as there is no way to resolve it with a WinNT:// style moniker" do
      user = Oregano::Util::Windows::SID::NtLocal
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::Group.exists?(user)).to be_truthy
    end
    ...
    it "should return nil with an unknown SID" do
      ...
      # ensure that the underlying OS is queried here
      Oregano::Util::Windows::ADSI.unstub(:connect)
      expect(Oregano::Util::Windows::ADSI::Group.exists?(bogus_sid)).to be_falsey
    end

    it "should be able to delete a group" do
      connection.expects(:Delete).with('group', groupname)

      Oregano::Util::Windows::ADSI::Group.delete(groupname)
    end

    it "should return an enumeration of IADsGroup wrapped objects" do
      name = 'Administrators'
      wmi_groups = [stub('WMI', :name => name)]
      Oregano::Util::Windows::ADSI.expects(:execquery).with('select name from win32_group where localaccount = "TRUE"').returns(wmi_groups)

      native_group = stub('IADsGroup')
      native_group.expects(:Members).returns([stub(:Name => 'Administrator')])
      Oregano::Util::Windows::ADSI.expects(:connect).with("WinNT://./#{name},group").returns(native_group)

      groups = Oregano::Util::Windows::ADSI::Group.to_a
 ...
 "Delete (WIN32OLERuntimeError)
    ...
    HRESULT error code:0x80020009
      ...
      Oregano.expects(:warning).with("Cannot delete user profile for 'S-A-B-C' prior to Vista SP1")
      Oregano::Util::Windows::ADSI::UserProfile.delete('S-A-B-C')
    end
  end
end
